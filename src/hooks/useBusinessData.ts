...
export function useBusinessData(
  ...
  const handleAddBusiness = useCallback(async () => {
    ...
    try {
      await addDoc(collection(db, 'businesses'), {
        ...
        serviceRadius: typeof state.formData.serviceRadius === 'number' ? state.formData.serviceRadius : 25,
        specialtyTags: state.formData.specialtyTags,
        cuisineTypes: state.formData.cuisineTypes || [],
  ...
  const handleStartEdit = useCallback(() => {
    ...
    dispatch({ type: 'SET_EDIT_DATA', payload: {
      ...
      cuisineTypes: state.selectedBusiness.cuisineTypes || [],
  ...
  const handleSaveEdit = useCallback(async () => {
    ...
    try {
      ...
      await updateDoc(ref, {
        ...
        cuisineTypes: state.editData.cuisineTypes || [],
