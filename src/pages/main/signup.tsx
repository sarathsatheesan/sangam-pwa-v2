...
import { AVATAR_OPTIONS, BUSINESS_TYPES, ETHNICITY_HIERARCHY, ETHNICITY_CHILDREN, PRIORITY_ETHNICITIES } from '../../constants/config';
  ...
  const renderStep2 = () => (
    <div>
      ...
      <div className="mb-8">
          ...
          <div className="space-y-2">
            {PRIORITY_ETHNICITIES.map((priorityItem) => {
              ...
              if (regionMatch) {
                ...
                const selectedInRegion = regionMatch.subregions.reduce((sum, sub) => sum + sub.ethnicities.filter((e) => formData.heritage.includes(e)).length, 0);
                return (
                  <div key={`pq-${priorityItem}`}>
                    <button
                      ...
                      className="w-full px-3 py-2 flex items-center justify-between hover:bg-amber-100/50 dark:hover:bg-amber-500/20 rounded-lg transition-colors text-xs"
                    >
                              ...
                              {isSubExpanded && (
                                <div className="ml-2 mt-1 space-y-1">
                                  {sub.ethnicities.map((eth) => {
                                    ...
                                    if (children) {
                                      const selectedChildren = children.filter((c) => formData.heritage.includes(c));
                                        ...
                                        <div key={`pq-${eth}`}>
                                            ...
                                            <input
                                              ...
                                              onChange={() => {
                                                if (selectedChildren.length === children.length) {
                                                  updateFormData('heritage', formData.heritage.filter((x: string) => !children.includes(x)));
                                                } else {
                                                  updateFormData('heritage', [...formData.heritage, ...children.filter((c) => !formData.heritage.includes(c))]);
                                                }
                                              }}
                                          ...
                                          {isEthExpanded && (
                                            <div className="ml-3 mt-0.5 space-y-0.5 text-xs">
                                              {children.map((child) => (
                                                <label key={`pq-${child}`} className="flex items-center gap-3 py-0.5 px-1 cursor-pointer hover:bg-amber-100/50 dark:hover:bg-amber-500/20 rounded">
                                                  <input
                                                    type="checkbox"
                                                    checked={formData.heritage.includes(child)}
                                                    onChange={() =>
                                                      updateFormData('heritage',
                                                        formData.heritage.includes(child)
                                                          ? formData.heritage.filter((h: string) => h !== child)
                                                          : [...formData.heritage, child]
                                                      )
                                                    }
                                                    className="w-3 h-3 text-amber-600 border-amber-300 rounded"
                                                  />
                                    ...
                                    return (
                                      <label key={`pq-${eth}`} className="flex items-center gap-3 py-0.5 px-1 cursor-pointer hover:bg-amber-100/50 dark:hover:bg-amber-500/20 rounded text-xs">
                                        <input
                                          type="checkbox"
                                          ...
                                          onChange={() =>
                                            updateFormData('heritage',
                                              formData.heritage.includes(eth)
                                                ? formData.heritage.filter((h: string) => h !== eth)
                ...
                return (
                  <div key={`pq-${priorityItem}`}>
                    <div className="flex items-center gap-3 py-2 px-3 hover:bg-amber-100/50 dark:hover:bg-amber-500/20 rounded-lg transition-colors">
                      <input
                        type="checkbox"
                        ref={(el) => { if (el) el.indeterminate = selectedChildren.length > 0 && selectedChildren.length < children.length; }}
                        checked={selectedChildren.length === children.length}
                    ...
                    {isExpanded && (
                      <div className="ml-2 mt-1 space-y-1 bg-amber-100/30 dark:bg-amber-500/10 rounded-lg p-2">
                        {children.map((child) => (
                          <label key={`pq-${child}`} className="flex items-center gap-3 py-0.5 px-1 cursor-pointer hover:bg-amber-100/50 dark:hover:bg-amber-500/20 rounded text-xs">
                            <input
                              type="checkbox"
                              checked={formData.heritage.includes(child)}
                              onChange={() =>
                ...
                return (
                  <label key={`pq-${priorityItem}`} className="flex items-center gap-3 py-2 px-3 hover:bg-amber-100/50 dark:hover:bg-amber-500/20 rounded-lg cursor-pointer transition-colors text-xs">
                    <input
                      type="checkbox"
                      checked={formData.heritage.includes(priorityItem)}
                      onChange={() =>
                        updateFormData('heritage',
                          formData.heritage.includes(priorityItem)
                            ? formData.heritage.filter((h: string) => h !== priorityItem)
                            : [...formData.heritage, priorityItem]
                        )
                      }
                      className="w-4 h-4 text-amber-600 border-amber-300 rounded"
                    />
        ...
        <div className="flex items-center gap-2 mb-4">
          <div className="flex-1 border-t border-dashed border-aurora-border"></div>
