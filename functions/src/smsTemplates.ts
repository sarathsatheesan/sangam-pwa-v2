...
const SMS_TEMPLATES: Record<string, (d: TemplateData) => string> = {

  // ── ORDER LIFECYCLE ───────────────────────────────────────────────

  new_order: (d) =>
    `${BRAND_PREFIX} New order! ${d.customerName} placed a ${d.headcount}-guest order (${fmtPrice(d.total)}). Open the app to confirm.`,
  ...
  quote_request_submitted: (d) =>
    `${BRAND_PREFIX} Quote request submitted for ${d.headcount} guests on ${d.eventDate}. Vendors will respond soon.`,

  vendor_quote_received: (d) =>
    `${BRAND_PREFIX} ${d.vendorName} quoted ${fmtPrice(d.totalPrice)} for your catering. Open app to compare.`,

  quote_accepted: (d) =>
    `${BRAND_PREFIX} Your quote (${fmtPrice(d.totalPrice)}) was accepted by ${d.customerName}! Check your dashboard.`,

  vendor_new_rfq: (d) =>
    `${BRAND_PREFIX} New request: ${d.cuisineCategory} for ${d.headcount} guests in ${d.deliveryCity}. Submit a quote!`,

  vendor_rfq_edited: () =>
    `${BRAND_PREFIX} A customer updated their catering request. Please review and re-quote.`,

  quote_expired: () =>
    `${BRAND_PREFIX} Your quote request has expired. Create a new one anytime.`,

  quote_request_edited: (d) =>
    d.requiresRequote
  ...
  payment_received: (d) =>
    `${BRAND_PREFIX} Payment of ${fmtPrice(d.amount)} received for your catering order.`,

  payment_failed: (d) =>
    `${BRAND_PREFIX} Payment of ${fmtPrice(d.amount)} failed. Update your payment method to avoid cancellation.`,

  refund_processed: (d) =>
    `${BRAND_PREFIX} Refund of ${fmtPrice(d.amount)} processed. Allow 5-10 business days.`,

  // ── MODIFICATIONS ─────────────────────────────────────────────────

  order_modified: (d) =>
    `${BRAND_PREFIX} ${d.businessName} modified your order. Open the app to review and accept/reject.`,

  modification_rejected: (d) =>
    `${BRAND_PREFIX} ${d.customerName} rejected your order modification. Order reverted.`,

  // ── RECURRING ORDERS ──────────────────────────────────────────────

  recurring_order_reminder: (d) =>
    `${BRAND_PREFIX} Reminder: "${truncate(d.label, 30)}" from ${d.businessName} is scheduled for tomorrow. Edit or skip in the app.`,

  // ── REVIEWS ───────────────────────────────────────────────────────

  vendor_new_review: (d) =>
    `${BRAND_PREFIX} New ${d.rating}-star review for ${d.businessName} from ${d.reviewerName}. Check your dashboard.`,

  review_flagged: (d) =>
    `${BRAND_PREFIX} A review for ${d.businessName} was flagged: ${truncate(d.reason, 50)}.`,
};
 ...
 */
export function getSmsTemplate(template: string, data: TemplateData): SmsTemplateResult | null {
