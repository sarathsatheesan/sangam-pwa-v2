...
**Date:** April 16, 2026 (Last updated: **Session 69, Sep 3 2026** — see the dated `### Session NN` entries at the END of this file; they are the source of truth for current state. This header is metadata only — when they disagree, trust the newest session entry.)
**Repo:** https://github.com/sarathsatheesan/sangam-pwa-v2
**Latest Commit:** see `git log` / the newest session entry (header commit refs go stale — Session 39's sat here for months)
**Recent arc (Sessions 44–52):** full-app code review → Phase A (aggregation queries, reportError, per-route error boundaries) → Phase B (shared Modal + a11y, alert() purge) → Phase C (service layer for all 5 content modules, shared moderation, type burn-down, TWO 4-month-old forum bugs fixed) → unbounded-query remediation (forum votes via collectionGroup, pagination, comment caps, firebase.json indexes wired for the FIRST time) → Phase D (270-test suite, TZ + money-math fixes, messages.tsx decomposition 76→60 useState, domains 1–3 done).
...
**Session 35 focused on:** Service Radius + Google Places Address Autocomplete Across All Modules + Race Condition Fix + Establishment Search. Multi-part session covering a new business field, rewriting the Places autocomplete system, and fixing 3 layered bugs:

- **Service Radius Field on Signup Pages:** Added `serviceRadius` (number, 1–100 miles) to the business sign-up flow. Updated `BusinessExtras` interface in `src/services/auth.ts`, added the field to both `src/pages/main/signup.tsx` and `src/pages/auth/signup.tsx` with number input, `inputMode="numeric"`, min/max validation, "miles" suffix label, and helper text. Also added to `BusinessCreateModal.tsx`, `BusinessEditModal.tsx`, and `StepReview.tsx`. CSV import initializer in `businessRegistration.ts` defaults to `serviceRadius: 25`.

- **Google Places Address Autocomplete Across All Modules:** Rewrote `AddressAutocomplete.tsx` from scratch to use the modern `useGooglePlaces` hook (previously used legacy `AutocompleteService` directly which failed with `loading=async`). Integrated into 6 modules: `BusinessCreateModal.tsx`, `BusinessEditModal.tsx`, `events.tsx` (both create and edit forms), `housing.tsx`, and `CateringCheckout.tsx`. Each module auto-fills structured address components (street, city, state, zip) plus lat/lng when a suggestion is selected. The new component uses ARIA combobox/listbox roles, touch event handling for iOS Safari/Android Chrome (`touchstart` for outside-click dismiss, `touchend` on suggestions), Aurora CSS variables throughout, and z-50 for modal layering.

- **Race Condition Fix (loading=async):** The signup page showed "Address autocomplete unavailable" because `google.maps` wasn't fully bootstrapped when `script.onload` fired (a known issue with `loading=async`). Added `waitForGoogleMaps()` polling function that waits up to 5 seconds after `onload`, checking every 100ms for `google.maps.importLibrary` or `google.maps.places.AutocompleteService`. Other modules worked because by the time users navigated there, the script had time to bootstrap.

- **Legacy Places API Fallback:** Added dual API support to `useGooglePlaces.ts` — tries "Places API (New)" (`AutocompleteSuggestion.fetchAutocompleteSuggestions`) first, falls back to legacy "Places API" (`AutocompleteService`) if the new API isn't available. This ensures autocomplete works regardless of which API variant is enabled in Google Cloud Console.
...
- **Rebrand Round 2 — Missed Sangam/EthniZity references (commit `382c84f`):** After the initial 32-file rebrand, a thorough grep audit found 6 additional user-facing brand references that were missed. All fixed:
  - `src/components/layout/AppHeader.tsx:181` — styled logo text still showing `<span>Ethni</span><span>Zity</span>` → changed to `<span>e</span><span>NoVo</span>` (this was the "top left of main tile" the user reported)
  - `src/pages/admin.tsx:1290` — "Sangam Admin" → "eNoVo Admin"
  - `src/components/layout/AppHeader.tsx:404` — `tel:+1-800-SANGAM` → `tel:+1-800-ENOVO`
  - `src/components/layout/AppFooter.tsx:59` — `tel:+1-800-SANGAM` → `tel:+1-800-ENOVO`
  - `src/components/business/BusinessDetailModal.tsx:136` — share text "on Sangam" → "on eNoVo"
  - `src/constants/theme.ts:2` — comment "SANGAM" → "eNoVo"

- **Verified clean after round 2:** Zero remaining user-facing "Sangam", "EthniZity", or "ethniCity" brand references in `.ts`/`.tsx` files. All remaining instances are storage keys (`@sangam_cookie_consent`, `sangam_saved_posts`, `sangam_catering_order_form`, `sangam_e2e_v1_salt`, `sangam_social_to_community_migrated`, `ethniCity_*`, `ethnicity_*`) and CSS variable names (`--color-sangam-*`) — all intentionally preserved.

- **Also deliberately NOT changed (CSS variable names):**
  - `--color-sangam-bg`, `--color-sangam-text`, `--color-sangam-border`, etc. in `src/index.css` — these CSS custom properties are referenced throughout the entire codebase; renaming them would be a massive find-and-replace across all component styles with high regression risk and zero user visibility
...
- **CSS variable names `--color-sangam-*` preserved (not renamed)** — These CSS custom properties in `index.css` are referenced throughout all component styles. Renaming to `--color-enovo-*` would require updating every file that uses them — a massive refactor with zero user-visible benefit since CSS variable names are internal implementation details.

- **Phone number updated: `tel:+1-800-SANGAM` → `tel:+1-800-ENOVO`** — Both AppHeader (hamburger menu call link) and AppFooter (phone action) now use the rebranded phone vanity number.

- **All rebrand changes committed and pushed in 2 commits** — Commit `8bad39e` (initial 32+ file rebrand) + commit `382c84f` (round 2 cleanup: AppHeader logo, admin title, phone links, share text, theme comment). Both pushed to `origin/main`. Deployment requires `git pull` on Mac first.
...
### Session 35: Service Radius + Google Places Autocomplete + Race Condition Fix + Establishment Search

- **`useGooglePlaces` hook as single source of truth (not inline AutocompleteService)** — The original `AddressAutocomplete.tsx` directly created `AutocompleteService` instances, which never worked with `loading=async` scripts because the API wasn't bootstrapped at constructor time. Moving all Places logic into the `useGooglePlaces` hook (with `importLibrary('places')` + polling) ensures a single, battle-tested initialization path shared by all consumers.

- **Dual API support: new API first, legacy fallback** — Google Cloud Console projects may have "Places API" (legacy) or "Places API (New)" enabled. The hook tries `AutocompleteSuggestion.fetchAutocompleteSuggestions` first, falls back to `AutocompleteService.getPlacePredictions`. This makes the app resilient to either configuration without requiring specific API activation in the console.

- **`waitForGoogleMaps()` polling over single-check** — With `loading=async` on the Google Maps script tag, `script.onload` fires before `google.maps` is fully bootstrapped. A single check for `google.maps.importLibrary` at `onload` time fails on cold page loads (like direct navigation to `/auth`). Polling every 100ms for up to 5 seconds handles this race condition reliably without adding excessive delay.

- **`'establishment'` type added to all Places consumers** — The new Places API `includedPrimaryTypes` accepts multiple types simultaneously, so adding `'establishment'` alongside address types enables business name search AND address search in the same input. For the legacy API, which only accepts a single value from `['geocode', 'address', 'establishment', '(regions)', '(cities)']`, the `types` restriction was removed entirely so all result types are returned.

- **`serviceRadius` optional in types, required in UI** — `BusinessExtras.serviceRadius?: number` is optional because existing Firestore documents don't have it. But signup forms validate it as required (`if (!formData.serviceRadius) errors.serviceRadius = 'Required'`). This follows the established defensive Firestore pattern: types allow missing fields, UI enforces new fields going forward. Default value of 25 miles used in CSV import.

- **AddressAutocomplete as shared component with `onSelect` callback** — Rather than having each module implement its own Places integration, the `AddressAutocomplete` component encapsulates the hook, dropdown UI, and selection logic. Consumers just provide `onChange` (for typing) and `onSelect` (for structured address result). This was integrated into 6 modules with module-specific field mapping in the `onSelect` handler.

- **Touch event handling for cross-browser dropdown** — iOS Safari and Android Chrome require special touch event handling: `touchstart` (not `mousedown`) for outside-click detection to close the dropdown, `touchend` (not `click`) on suggestion items to prevent the keyboard from re-opening. The component handles both pointer types.
...
### Session 35 (April 26, 2026) — Service Radius + Google Places Autocomplete + Race Condition Fix + Establishment Search

**What we did:** Added serviceRadius field to business signup, rewrote Google Places address autocomplete across all modules, fixed a 3-layer bug preventing autocomplete from working on the signup page, and added establishment type support so business names return results.

**Feature: Service Radius (Max Service Distance)**

| Task | File(s) Changed | Status |
|------|----------------|--------|
| Add `serviceRadius?: number` to BusinessExtras interface | `src/services/auth.ts` | ✅ |
| Add serviceRadius field to main signup page | `src/pages/main/signup.tsx` | ✅ |
| Add serviceRadius field to auth signup page | `src/pages/auth/signup.tsx` | ✅ |
| Add serviceRadius to BusinessCreateModal | `src/components/business/BusinessCreateModal.tsx` | ✅ |
| Add serviceRadius to BusinessEditModal | `src/components/business/BusinessEditModal.tsx` | ✅ |
| Add serviceRadius to StepReview display | `src/components/business/registration/StepReview.tsx` | ✅ |
| Default serviceRadius:25 in CSV import | `src/services/businessRegistration.ts` | ✅ |

**Feature: Google Places Address Autocomplete — All Modules**

| Task | File(s) Changed | Status |
|------|----------------|--------|
| Rewrite AddressAutocomplete to use useGooglePlaces hook | `src/components/shared/AddressAutocomplete.tsx` | ✅ |
| Integrate into BusinessCreateModal (Location/Address) | `src/components/business/BusinessCreateModal.tsx` | ✅ |
| Integrate into BusinessEditModal (Location/Address) | `src/components/business/BusinessEditModal.tsx` | ✅ |
| Integrate into Events — create form (Venue/Address) | `src/pages/events.tsx` | ✅ |
| Integrate into Events — edit form (Venue/Address) | `src/pages/events.tsx` | ✅ |
| Integrate into Housing (Address) | `src/pages/housing.tsx` | ✅ |
| Fix CateringCheckout AddressAutocomplete integration | `src/components/catering/CateringCheckout.tsx` | ✅ |

**Bug Fix: Race Condition + Legacy Fallback + Establishment Type**

| Task | File(s) Changed | Status |
|------|----------------|--------|
| Add `waitForGoogleMaps()` polling for loading=async race condition | `src/hooks/useGooglePlaces.ts` | ✅ |
| Add legacy Places API (AutocompleteService) fallback | `src/hooks/useGooglePlaces.ts` | ✅ |
| Add `'establishment'` type to all 4 call sites | `useGooglePlaces.ts`, `AddressAutocomplete.tsx`, `auth/signup.tsx`, `StepLocation.tsx` | ✅ |
| Remove legacy API `types: ['address']` restriction | `src/hooks/useGooglePlaces.ts` | ✅ |
| Fix TS7009 strict errors on dynamic constructors | `src/hooks/useGooglePlaces.ts` | ✅ |

**Also completed this session (from earlier in the session, prior to context compaction):**

| Task | File(s) Changed | Status |
|------|----------------|--------|
| RFQ interception modal for large/advance orders | `src/components/catering/RFQInterceptionModal.tsx` (NEW) | ✅ |
| Fix scroll jump on radio button focus in OrderForSelector | `src/components/catering/CateringCheckout.tsx` | ✅ |
| Replace native radio inputs with ARIA radios | `src/components/catering/CateringCheckout.tsx` | ✅ |
| Standardize date picker API across RFP and Order modules | Multiple catering files | ✅ |
| Make RFP event time required + platform-wide appearance:auto | Multiple files | ✅ |
| Comprehensive validation error messages for RFP form | `src/components/catering/RequestForPriceForm.tsx` | ✅ |
...
0ceaf37 fix: add legacy Places API fallback for address autocomplete
9c0212c feat: enable Google Places address autocomplete across all modules
49c9def feat: add serviceRadius to initial business sign-up pages
ebf1ab6 fix: add missing serviceRadius default to CSV row initializer
b283d68 feat: add Maximum Service Radius field to all business entry points
aa065ba Add comprehensive validation error messages to RFP form
f95e375 feat: make RFP event time required + platform-wide appearance:auto patch
1858873 feat: standardize date picker API across RFP and Order modules
b141c4c fix: replace native radio inputs with ARIA radios to eliminate scroll jump
968723b fix: prevent scroll jump on radio focus in OrderForSelector
894859c fix: prevent scroll jump on radio button focus in OrderForSelector
ef85ac3 feat: add RFQ interception modal during checkout for large/advance orders
```
...
- **Service Radius Field:** ✅ COMPLETE (Session 35) — `serviceRadius` (1–100 miles) added to BusinessExtras interface, both signup pages (main/signup.tsx, auth/signup.tsx), BusinessCreateModal, BusinessEditModal, StepReview, CSV import default (25 miles). Number input with inputMode="numeric", min/max, "miles" suffix.
- **Google Places Address Autocomplete — All Modules:** ✅ COMPLETE (Session 35) — Rewrote AddressAutocomplete.tsx to use useGooglePlaces hook. Integrated into BusinessCreateModal, BusinessEditModal, events.tsx (create + edit), housing.tsx, CateringCheckout.tsx. ARIA combobox/listbox, touch events for iOS/Android, Aurora CSS vars, z-50 modal layering.
- **Google Places Race Condition Fix:** ✅ COMPLETE (Session 35) — Added `waitForGoogleMaps()` polling (100ms intervals, 5s timeout) to handle `loading=async` bootstrap delay. Fixed signup page "Address autocomplete unavailable" error.
- **Legacy Places API Fallback:** ✅ COMPLETE (Session 35) — Dual API support in useGooglePlaces.ts: tries Places API (New) first, falls back to legacy AutocompleteService.
- **Establishment Type in Places Autocomplete:** ✅ COMPLETE (Session 35) — Added `'establishment'` to `includedPrimaryTypes` in all 4 call sites so business name search (e.g., "India Cultural Center") works alongside address search. Legacy API types restriction removed.
...
- **Test address autocomplete in ALL modules**: (1) Business Create modal — type a business name (e.g., "India Cultural Center") and an address → both should return results. (2) Business Edit modal — same test. (3) Events create + edit forms — type address, verify city/state/zip auto-fill. (4) Housing — type address, verify auto-fill. (5) Catering Checkout — type address, verify auto-fill. (6) Signup page (`/auth`) — type address in business address field → autocomplete should appear (no "Address autocomplete unavailable" error). (7) Business Registration wizard StepLocation — type address, verify results.
- **Cross-browser verification**: Test address autocomplete on Chrome, Safari (desktop + iOS), Firefox, and Android Chrome. Verify dropdown appears, suggestions are tappable on touch devices, and outside-click dismisses the dropdown.
...
| `src/hooks/useGooglePlaces.ts` | Google Places Autocomplete hook — singleton script loader with `waitForGoogleMaps()` polling for `loading=async` race condition, dual API support (new API `AutocompleteSuggestion` + legacy `AutocompleteService` fallback), session tokens for billing, `getPlacePredictions` + `getPlaceDetails` with structured address parsing. Types include `'establishment'` for business name search. REWRITTEN in Session 35 (originally Session 21). |
| `src/components/shared/AddressAutocomplete.tsx` | Shared address autocomplete component wrapping `useGooglePlaces` hook. ARIA combobox/listbox roles, touch event handling (iOS Safari/Android Chrome), Aurora CSS vars, z-50 for modal layering. Used by BusinessCreateModal, BusinessEditModal, events.tsx, housing.tsx, CateringCheckout.tsx. REWRITTEN in Session 35. |
...
- **Google Maps `loading=async` race condition** — When the script tag uses `loading=async`, `script.onload` fires BEFORE `google.maps` is fully bootstrapped. `google.maps.importLibrary` may not exist yet. The `waitForGoogleMaps()` function in `useGooglePlaces.ts` polls every 100ms (up to 5 seconds) until the API is ready. This is critical for pages loaded via direct navigation (e.g., `/auth`) where the script hasn't had time to bootstrap. Never remove this polling or replace it with a single check at `onload` time.
- **`includedPrimaryTypes` must include `'establishment'` for business name search** — Without it, typing a business name (e.g., "India Cultural Center") in the Location/Address field returns only street address matches. The `'establishment'` type is set in 4 places: `useGooglePlaces.ts` default, `AddressAutocomplete.tsx`, `auth/signup.tsx`, `StepLocation.tsx`. If you add a new consumer, include `'establishment'` in the types array.
- **Legacy Places API `types` parameter accepts only ONE value** — The legacy `AutocompleteService.getPlacePredictions` `types` option accepts `['geocode']`, `['address']`, `['establishment']`, `['(regions)']`, or `['(cities)']` — only one at a time. To get both addresses AND establishments, omit the `types` parameter entirely (returns all types). The new API's `includedPrimaryTypes` accepts multiple values simultaneously.
- **`AddressAutocomplete` needs `onSelect` AND `onChange`** — `onChange` fires on every keystroke (for the raw text value), `onSelect` fires when the user picks a suggestion (provides structured `AddressResult` with street, city, state, zip, lat, lng). Consumers should map `onSelect` to their specific form state shape.
...
```
(latest) 5e76d3d fix: include establishment type in Places autocomplete for business name search (Session 35)
d0f715b fix: resolve race condition in Google Places initialization on signup page (Session 35)
4f61d62 fix: resolve race condition in Google Places initialization on signup page (Session 35 — TS type fixes)
0ceaf37 fix: add legacy Places API fallback for address autocomplete (Session 35)
9c0212c feat: enable Google Places address autocomplete across all modules (Session 35)
49c9def feat: add serviceRadius to initial business sign-up pages (Session 35)
ebf1ab6 fix: add missing serviceRadius default to CSV row initializer (Session 35)
b283d68 feat: add Maximum Service Radius field to all business entry points (Session 35)
aa065ba Add comprehensive validation error messages to RFP form (Session 35)
f95e375 feat: make RFP event time required + platform-wide appearance:auto patch (Session 35)
1858873 feat: standardize date picker API across RFP and Order modules (Session 35)
b141c4c fix: replace native radio inputs with ARIA radios to eliminate scroll jump (Session 35)
968723b fix: prevent scroll jump on radio focus in OrderForSelector (Session 35)
894859c fix: prevent scroll jump on radio button focus in OrderForSelector (Session 35)
ef85ac3 feat: add RFQ interception modal during checkout for large/advance orders (Session 35)
f4ad546 fix: raise esbuild target from safari14 to safari15 to fix destructuring build error (Session 34)
...
<!--
  ...
  Last updated: Session 52 (July 3, 2026) — newest entries are appended at the END of this file
-->

*Updated May 30, 2026 (Session 39) — for continuing development in a new session. Business Module ALL 42 roadmap items COMPLETE (Sessions 11-18). Discover Page ALL 38 items COMPLETE (Sessions 19-20). Business Sign-Up Wizard ALL 5 phases COMPLETE (Session 21). Catering Module Phases 1-6 ALL COMPLETE (Sessions 22-23). UX Audit + Critical Fixes COMPLETE (Session 24). Medium+Large Effort Features + Architecture Refactor ALL COMPLETE (Session 25). Round 2 Service Blueprint Audit ALL 30 findings COMPLETE (Session 26). UI Overhaul + QA + 21 Bug Fixes ALL COMPLETE (Session 27). RFP-to-Order + In-Order Messaging ALL COMPLETE (Session 28/28b). Vendor Storefront Builder + 10 Bug Fixes ALL COMPLETE (Session 29). Multi-Business Architecture + QA Audit (48 tests) + UI Flicker Fix ALL COMPLETE (Session 30). Vendor Dashboard Header Refactor + Dual-Role Nav Fix + "My Orders" Rename ALL COMPLETE (Session 30b). Notification Deep-Link Routing ALL COMPLETE (Session 31). Handoff Note Consolidation + White-Page QA Audit + ErrorBoundary & Null Guard Fixes ALL COMPLETE (Session 32). Profile Page Enhancements + Admin Avatar Fix + Contextual CTAs + Deep-Link Creation Forms + Heritage Selector Standardization + Karpathy Coding Skill + Full-App Karpathy Code Review ALL COMPLETE (Session 32+). Karpathy Code Review Implementation — all 4 sprints COMPLETE (Session 32++). God-Component Decomposition — 3 components decomposed + cross-browser patches + feature audit + 2 critical fixes ALL COMPLETE (Session 33). Catering Event Time + Cross-Browser Audit (23 date/time inputs, showPicker API) + Cart UX Fix + esbuild Build Target Fix ALL COMPLETE (Session 34). Service Radius + Google Places Autocomplete + Race Condition Fix + Establishment Search ALL COMPLETE (Session 35). QA Bug Fix Verification (B1 CSV template + B2 dead category condition) + Cuisine Dropdown Live Testing + RFP Confirmation Modal + Price Guidance — all 3 implementation phases VERIFIED COMPLETE (Session 36). Phase 4 Code Review (i18n + component extraction + security + tests + monitoring) ALL COMPLETE + Missing Firestore Composite Index Fixed + Vendor Order Status Flow Restored (Session 37). Reprice UX Rework — scoped repriceItemNames + delivery fee separation + vendor view updates + cross-browser patches ALL COMPLETE (Session 38). Custom Domain Setup (5 domains on Firebase via IONOS) + Full Rebrand EthniZity → eNoVo (32+ files, ~200+ occurrences) + Rebrand Round 2 (6 missed Sangam/EthniZity references) ALL COMPLETE — committed as `8bad39e` + `382c84f` and pushed to origin/main (Session 39). Needs deploy from macOS: `cd /Users/sarathsatheesan/ethniCity_03_19_2026/sangam-pwa-v2 && git pull origin main && rm -rf node_modules package-lock.json && npm install && npm run build && firebase deploy --only hosting` (rollup platform binary mismatch — Linux sandbox installed linux-x64-gnu, Mac needs darwin-arm64). Critical context: **Brand is now "eNoVo" (mixed-case stylization)** — tagline "Our culture, connected." — email domain @enovoapp.com — phone `tel:+1-800-ENOVO`; localStorage/IndexedDB keys still use old names (intentional — data preservation): `@sangam_cookie_consent`, `sangam_saved_posts`, `sangam_catering_order_form`, `sangam_e2e_v1_salt`, `ethniCity_selected_business_`, `ethniCity_catering_cart`, `ethniCity_recent_business_searches`, `ethnicity_e2ee`, `ethnicity_checkout_form`, `ethnicity_vendor_*`, `ethnicity_rfq_walkthrough_seen`; CSS vars `--color-sangam-*` in index.css also preserved (renaming would touch every component); component/variable/route names unchanged; "Heritage" replaces "EthniZity" as user-facing label for the ethnicity filter concept; 5 custom domains active: enovoapp.com, ethnizity.com, enovoapp.online, enovoapp.org, enovoapp.store; www.enovoapp.com needs separate Firebase custom domain entry (shows SSL error without it); DNSSEC must stay OFF on IONOS domains for Firebase ACME challenges; ToastContext exposes `addToast` NOT `showToast`; Firestore rejects `undefined` — always strip; aurora CSS vars that DON'T exist: --aurora-primary, --aurora-green, --aurora-bg-secondary, --aurora-text-primary, --aurora-accent; useBusinessSwitcher() API: { businesses, selectedBusiness, selectBusiness, loading, isMultiBusiness }; CateringNotification now has `role?: 'vendor' | 'customer'` field; VENDOR_TYPE_FALLBACK set: new_order, modification_rejected, quote_accepted, item_reassigned, rfp_edited, reprice_requested; BOTH_PARTY_TYPES: order_cancelled, reprice_resolved (use body-text heuristic for legacy); App.tsx has `AppErrorBoundary` wrapping Routes + `lazyRetry()` on all 24 lazy imports (Session 32); always use `(field || '').substring()` for Firestore text and `(array || []).map()` for Firestore arrays; dietary tags snake_case; categories Title Case; `archived` not `isArchived`; ParsedMenuItem.price in cents; PWA SW cache can serve stale bundles — must build before deploy; `ctaConfig` record in profile.tsx maps listingsFilter → CTA label/route (Session 32+); `isImageUrl()` + `AvatarImg` helper in admin.tsx for safe avatar rendering (Session 32+); `CountryEthnicitySelector` is single source of truth for heritage — uses `"Country - Ethnicity"` scoped key format; `?action=add` deep-link pattern on business, marketplace, events pages triggers creation modal auto-open; Karpathy coding skill at `.claude/skills/karpathy-coding/SKILL.md` — auto-loads in Cowork sessions; God-component decomposition plan at `docs/god-component-decomposition-plan.md`; admin panels in `src/components/admin/panels/` receive state+callbacks as props from `admin.tsx`; message components in `src/components/messages/` are self-contained UI-only; catering sub-components in `src/components/catering/vendor/` — OrderCard is NOW ACTIVE (Session 37, no longer dead code), PendingOrdersSection and ActiveOrdersSection pass OrderCardProps; OrderCard manages edit state + ETA state internally via useState (not props); `formatEtaValue()` lives inside OrderCard; vendor status progression: pending → confirmed → preparing → ready → out_for_delivery → delivered; Firestore composite indexes: cateringOrders needs BOTH (businessId+createdAt) AND (customerId+createdAt) — missing index causes silent subscription failure; fallback query pattern in cateringOrders.ts retries with simple where() + client-side sort if indexed query fails; `verbatimModuleSyntax` in tsconfig requires `import type` for type-only imports; build target is `safari15` (raised from safari14 in Session 34 — esbuild can't transform destructuring for safari14); `showPicker()` API used on all 23 date/time inputs for cross-browser native picker popup; `eventTime` stored as HH:MM 24h, displayed as 12h AM/PM — optional in Firestore types but required at checkout; Notification.requestPermission() uses Promise wrapper for Safari < 16.4 callback API compat; CUISINE_CATEGORY_KEYS — 17 hardcoded cuisine types; `availableCuisineKeys` in RequestForPriceForm.tsx filters by reachable vendors with graceful fallback to all 17; food category condition: `'Restaurant & Food' || 'Tiffin' || 'Grocery & Market'` (NOT "Restaurant & Food Catering" — that category doesn't exist); `getDistanceMiles()` Haversine formula in businessUtils.ts; `serviceRadius` defaults to 25 miles; `repriceRequestedPrice` and `repriceCounterPrice` are ITEMS-ONLY subtotals (excludes delivery fee); `repriceItemNames?: string[]` tracks which items were repriced; `respondToReprice()` and `resolveCounterOffer()` add `deliveryFee` to compute `total`; all reprice UI shows items + delivery = total; `requestReprice()` accepts optional `repriceItemNames` parameter; cross-browser: `inputMode="decimal"`, `WebkitAppearance: 'none'`, `MozAppearance: 'textfield'`, `-webkit-overflow-scrolling: touch` on all reprice inputs/modals. Design tokens: primary #6366F1, error #EF4444, success #22c55e. Aurora theme: light surfaces #F5F6FA/#FFFFFF, dark surfaces #1A1B2E/#232438. Next: (1) deploy from macOS: `git pull origin main && rm -rf node_modules package-lock.json && npm install && npm run build && firebase deploy --only hosting`, (2) add `www.enovoapp.com` as separate custom domain in Firebase Console, (3) end-to-end test reprice flow with scoped items (select 1 of 3 items, verify only that item in accept CTA), (4) end-to-end test vendor order status progression (confirmed→preparing→ready→dispatch→delivered), (5) test customer order visibility after RFP acceptance, (6) set cuisineTypes on existing businesses to activate real filtering, (7) end-to-end test RFP with active cuisine filter, (8) test all admin panels (moderation + registrations), (9) clean up dead etaInputs/formatEtaValue in VendorCateringDashboard, (10) Phase 7 UX improvements / sort-filter controls on marketplace. Long-term: payment integration, multi-vendor cart, driver tracking, recommendation engine.*

---

### Session 40 (May 30, 2026) — Android App via Capacitor + Web Push/Manifest/TURN Fixes + Branding + Play Store Prep + App Links + git commit/push

> Worked in the `sangam-pwa-v2` repo (the actual app). A separate per-session note `HANDOFF05302026.md` (Session 39) pointed here and was partially stale — see corrections below.

**(1) What we were building/working on**
Packaging the existing eNoVo PWA as a native **Android app for Google Play without rewriting the web codebase**. Along the way: fixed web issues found during inspection (broken web push, PWA manifest mismatch, free TURN), generated app branding (icon/splash), wrote the privacy policy + Play submission kit, got the app **running on a physical Samsung tablet**, produced a **signed AAB**, verified **App Links**, and committed/pushed everything. Then started planning new feature modules (deferred Play submission until business registration).

**(2) Decisions made & why**
- **Capacitor** chosen over TWA and React Native. RN rejected = full rewrite (violates "keep the web codebase"). TWA rejected because it **cannot** show a native full-screen incoming-call UI, **cannot** run a foreground service to keep calls alive, and only does best-effort web push — unacceptable for a calling-centric app. Capacitor wraps the exact `dist/` build in a native shell, keeps WebRTC in the System WebView, and enables native FCM + a path to native call UX. Cost: one native project + `npx cap sync` per release.
- **appId `enovoapp.app`** (permanent, user choice; valid two-segment package name).
- **Native FCM** for push (not web push in the wrapper) — high-priority/background delivery, channels, call wake.
- **Web kept byte-for-byte behaviorally identical**: only one guarded line added to `src/main.tsx` (`void initNative()`), which no-ops on web via `Capacitor.isNativePlatform()===false`. (User explicitly required the web version stay intact.)
- **MVP call notifications now; native full-screen call screen as a fast-follow.**
- **PWA manifest:** set `manifest: false` in `vite.config.ts` so the eNoVo `public/manifest.json` is the single source (the plugin was emitting a second, stale "ethniCity" manifest).
- **TURN:** centralized, env-driven config; free `openrelay` is now only a DEV fallback (it drops calls on cellular/symmetric NAT — must replace before launch).
- **Splash background `#091827` (navy)** to match the existing logo artwork so the logo blends seamlessly.
- **Play deferred until business registration** → an **Organization** Play account skips the new 12-tester/14-day closed-testing requirement (see next steps).

**(3) Completed**
- Capacitor Android project added; **app builds, installs, and runs on a physical Samsung Galaxy Tab A9+ (SM-X230)** (splash → web app). Reinstalled after App Links deploy.
- `src/native/{index,push,deepLinks}.ts`: native FCM token → existing `users/{uid}.fcmTokens` array; notification-tap routing mirroring `firebase-messaging-sw.js`; App Links + hardware back-button handling. One guarded line in `main.tsx`.
- `package.json`: added `@capacitor/*` deps + scripts (`cap:sync`, `cap:open`, `android:assets`).
- `AndroidManifest.xml`: CAMERA, RECORD_AUDIO, MODIFY_AUDIO_SETTINGS, POST_NOTIFICATIONS, FOREGROUND_SERVICE(+_MICROPHONE) + `enovoapp.com`/`www.enovoapp.com` App Links intent-filter (`autoVerify`).
- Firebase **Android app registered in project `mithr-1e5f4`**; `google-services.json` placed (gitignored); google-services plugin auto-applies via the Capacitor template's conditional block.
- **Signing**: upload keystore generated (`android-keys/enovo-upload.jks`, alias `enovo-upload`); wired in `android/app/build.gradle` via `android/keystore.properties` (gitignored). **Signed AAB built**: `android/app/build/outputs/bundle/release/app-release.aab` (19 MB).
- **App Links verified**: `assetlinks.json` deployed and confirmed by Google's Digital Asset Links API. Added a `firebase.json` rewrite `/.well-known/assetlinks.json → /assetlinks-android.json` so it serves despite the `"**/.*"` dotfile ignore + SPA catch-all.
- **Web push fixed**: VAPID key now read from `VITE_FIREBASE_VAPID_KEY` (was placeholder `PENDING_VAPID_KEY`); real key set in `.env`.
- **PWA manifest mismatch fixed** (`manifest:false`).
- **TURN centralized**: `src/utils/iceConfig.ts` (env-driven, `getIceServers()` stub for future ephemeral creds) used by `webrtc.ts` + `groupWebrtc.ts`.
- **Branding**: `resources/icon.png` (1024) + `splash.png`/`splash-dark.png` (2732, navy) generated from `public/icon-512.png`; applied via `npm run android:assets`. Splash bg `#091827`.
- **Privacy policy**: `public/privacy.html` → serves at `https://enovoapp.com/privacy.html`.
- **Docs**: `ANDROID_SETUP.md` (full run/build/sign walkthrough) + `PLAY_SUBMISSION.md` (store listing copy, Data Safety answers mapped to actual data, content rating, Internal/Closed testing steps).
- **Committed `f36d252` and pushed to `origin/main`** (111 files; secrets verified excluded).
- Environment fix during the session: Android Studio had **no SDK and no Gradle JDK** configured (fresh install) — installed via Settings → Android SDK + set Gradle JDK; AGP auto-upgraded 8.7.2→8.13.2, Gradle→8.13.

**(4) In progress / half-done**
- **Push verification incomplete**: on Samsung the Android-13 notification prompt didn't appear (Samsung likely auto-granted). Token registration into `users/{uid}.fcmTokens` not yet explicitly confirmed (Logcat/Firestore) and no real push test run.
- **Play Store NOT submitted** — deferred to after business registration (Organization account). AAB ready; shareable APK not yet built.
- **Second App Links fingerprint pending**: only the **upload-key** SHA-256 is in `assetlinks.json` (works for sideloaded builds). Must add the **Play App Signing** SHA-256 (Play Console → Setup → App integrity) after first AAB upload for Play-installed users.
- **Module planning started, nothing spec'd**: proposed 4 modules — Groups & sub-communities, Jobs & gigs, Services marketplace, Newcomer/settlement hub. Recommended building Groups first.

**(5) Exact next steps**
1. (APK) `cd android && ./gradlew assembleRelease` → `app-release.apk` for direct sharing/sideload (note: sideloaded APKs don't auto-update; Samsung Auto Blocker + Play Protect warn).
2. (Push) Confirm FCM token registers (Logcat tag `[native/push]` or check Firestore `users/{uid}.fcmTokens`), then send a test push from Firebase console.
3. (TURN) Provision production TURN (metered.ca paid / Twilio NTS / Cloudflare / self-host coturn) → set `VITE_TURN_URLS`/`_USERNAME`/`_CREDENTIAL` in `.env`; or implement ephemeral creds in `getIceServers()` (`iceConfig.ts`) + a Cloud Function. Free openrelay drops cellular calls.
4. (Web) Redeploy hosting to publish privacy.html + assetlinks + VAPID push: `npm run build && firebase deploy --only hosting`.
5. (FCM SW) Update `public/firebase-messaging-sw.js` notification-click host (hardcoded `mithr-1e5f4.web.app`/`localhost`) to include `enovoapp.com`.
6. (Play — after business registration) Create **Organization** Play account → Internal testing → upload AAB → grab **Play App Signing SHA-256** → add as 2nd `assetlinks.json` fingerprint + redeploy → Closed testing (12 testers/14 days only if Personal account) → Production.
7. (Modules) Spec **Groups & sub-communities** first, or produce a Now/Next/Later roadmap across the 4 proposed modules.

**(6) Important context, constraints & files**
- **Firebase project is `mithr-1e5f4`** (NOT `ethnicity-app` — Session-39 `HANDOFF05302026.md` was WRONG; `.firebaserc`, `src/services/firebase.ts`, and the FCM SW all use `mithr-1e5f4`).
- **Builds are Mac-only** (Rollup/Gradle platform binaries; the Cowork Linux sandbox can read/grep/edit and run `tsc` but NOT `npm run build` or Gradle).
- **Secrets gitignored — NEVER commit**: `.env`, `android/keystore.properties`, `android/app/google-services.json`, `android-keys/`.
- **Keystore**: `android-keys/enovo-upload.jks`, alias `enovo-upload`; store & key password identical (verify: `keytool -list -keystore android-keys/enovo-upload.jks`). **Upload-key SHA-256**: `D6:52:0F:39:67:C3:57:50:0F:A8:0C:F3:62:C8:54:1C:9D:39:6C:E0:C6:10:85:C5:3D:FB:55:3F:9F:22:CB:72`.
- **`androidScheme: 'https'`** in `capacitor.config.ts` is REQUIRED (secure context for getUserMedia + FCM) — do not change.
- WebRTC `getUserMedia` auto-grants in Capacitor's WebView for the app origin once OS camera/mic perms are granted (no MainActivity override needed for v1; fallback snippet in `android-setup/MainActivity.webrtc.snippet.java`).
- **Samsung specifics**: Developer mode under Settings → About tablet → Software information → tap Build number ×7. **Auto Blocker** (Settings → Security and privacy) blocks USB debugging/ADB and APK sideload — must be OFF. FCM needs **Google Play Services** (not Fire tablets).
- **Stale corrections to `HANDOFF05302026.md`** (already fixed in that file this session): no `tailwind.config.ts` (Tailwind **v4 CSS-first**, config in `src/index.css`); Firebase config is `src/constants/config.ts` + `src/services/firebase.ts` (no `src/config.ts`).
- **New/changed files this session**: NEW — `capacitor.config.ts`, `src/native/{index,push,deepLinks}.ts`, `src/utils/iceConfig.ts`, `android/` (whole native project), `android-setup/*` (reference snippets), `scripts/generate-keystore.sh`, `public/privacy.html`, `public/.well-known/assetlinks.json`, `public/assetlinks-android.json`, `resources/{icon,splash,splash-dark}.png`, `ANDROID_SETUP.md`, `PLAY_SUBMISSION.md`. EDITED — `src/main.tsx`, `src/pages/messages.tsx`, `src/utils/webrtc.ts`, `src/utils/groupWebrtc.ts`, `vite.config.ts`, `package.json`, `firebase.json`, `.gitignore`.

**Tech stack (carry-forward):** React 19.2 + TypeScript 5.9 (strict) + Vite 7 + `vite-plugin-pwa`; **Tailwind v4 (CSS-first, NO `tailwind.config.ts`)**; Firebase Auth (email/password + phone via reCAPTCHA — **no social OAuth**), Firestore, Storage, Cloud Functions, FCM; **Capacitor 7** for Android; react-router-dom 7, i18next, framer-motion, recharts, crypto-js (E2E messages), zod. AGP 8.13.2 / Gradle 8.13.

**Design system / brand / tone (carry-forward):**
- Brand **eNoVo** (mixed-case), tagline **"Our culture, connected."**, domain `enovoapp.com`. User-facing label for the ethnicity concept is **"Heritage."** Tone: warm, community-first, diaspora-focused.
- **Aurora palette** (source of truth `src/constants/theme.ts`): primary **Electric Indigo `#6366F1`** (light `#818CF8`, dark `#4F46E5`), secondary **Soft Mint `#10B981`**, tertiary **Aurora Cyan `#22D3EE`**, gradient indigo→mint. Light bg `#F5F6FA` / surface `#FFFFFF`; dark bg `#1A1B2E` / surface `#232438`. Text `#1E2132`/`#5B5E72`/`#9295A5`. Semantic: success `#10B981`, danger `#EF4444`, warning `#F59E0B`. **Logo artwork sits on navy `#091827`** (icon/splash bg).
- CSS vars keep `--color-sangam-*` prefix (intentional, in `src/index.css`); localStorage/IndexedDB keys keep `sangam_*`/`ethnicity_*` prefixes (data preservation). PWA `theme_color`/`background_color` `#F5F6FA`.

*Updated May 30, 2026 (Session 40) — Android app (Capacitor, appId `enovoapp.app`) running on physical Samsung tablet; signed AAB built; App Links verified (upload key); web push (VAPID), PWA manifest, and TURN centralization fixed; branding + privacy policy + Play submission kit created; all committed `f36d252` and pushed to origin/main. Play submission DEFERRED until business registration (Organization account skips the 12-tester/14-day rule). Immediate next: build shareable APK, verify FCM push on device, provision production TURN, redeploy hosting, then (post-registration) Play Internal testing + add Play App Signing SHA-256 to assetlinks. Then spec the Groups module. Firebase project is mithr-1e5f4. Builds are Mac-only.*

---

### Session 41 (May 30, 2026) — Message Module Bug Fixes: Calling (PiP, camera, drops) + Notifications + Unread Badges

**(1) What we were working on**
Fixing a list of **8 bugs** in the message module's voice/video calling + messaging, worked in grouped batches. 6 of 8 done; 2 deferred to next session. Plus three requested extras (foreground push toast, home Messages-tile unread badge, messages-list unread filter fix).

**(2) Decisions & why**
- **Grouped the PiP bugs (3, 6, 8)** since they share one DOM surface in `GlobalCallOverlay.tsx` — editing separately would conflict.
- **PiP gestures: switched Pointer Events → touch + mouse events.** Pointer Events worked in desktop Safari but NOT in Android's System WebView (Capacitor) — touch+mouse is the WebView-safe path. (User confirmed working on device.)
- **Call-drop recovery (Bug 5): grace window + caller-initiated ICE restart** over the existing Firestore call doc (new `restartOffer`/`restartAnswer` fields). Don't drop on transient `disconnected`; only end after a grace window. Flagged that a *consistent* threshold drop is most likely the free openrelay TURN allocation expiring → real fix is a production TURN.
- **Incoming-call notifications (Bug 1): NEW Cloud Function** `sendIncomingCallNotification` on `calls/{callId}` create. Before, calls had NO server-side push — only an in-app listener that needs the app open. MVP = high-priority heads-up + tap-to-open; full-screen ring while app killed is a future fast-follow.
- **Android notifications (Bug 7):** added `android: { priority, notification.channelId }` to the FCM payload (required for heads-up on Android 8+), created native channels (`messages`, `calls`, importance 5), fixed stale `click_action` host `mithr-1e5f4.web.app` → `enovoapp.com`.
- **Foreground toast:** Android delivers foreground pushes to the app (no auto banner), so added a window-event → in-app toast bridge (`NativePushToast`).
- **Unread badges via `lastMessageRead`, NOT `unreadCount`.** Root cause discovery: **`unreadCount` is dead** — only ever reset to 0 in messages.tsx, NEVER incremented on send. The reliable signal is `lastMessageRead` (set false on send @ messages.tsx:1351, true on read @ 802/845). Used `lastMessageRead === false && lastMessageSenderId !== uid`. This gives an unread-*conversation* count, not a message count.

**(3) Completed (6 of 8 bugs + extras)**
- **Bug 3** (minimized hides remote video): made PiP frame transparent for video + aligned the persistent `<video>` to a shared `pipPosition`/`pipDims` so video shows through. `GlobalCallOverlay.tsx`.
- **Bug 6** (minimized blocks composer): PiP default position seeded 120px above viewport bottom (clears composer) + draggable.
- **Bug 8** (PiP gestures): drag-to-move (4px threshold, clamped) + double-tap-to-expand; touch+mouse events (Android-safe). `GlobalCallOverlay.tsx`. **Device-confirmed working.**
- **Bug 5** (call drops): `webrtc.ts` — `handleConnected`/`handleConnectionTrouble`/`scheduleEndIfStillBroken`/`attemptIceRestart`; grace window + ICE restart; both call-doc listeners handle restart offer/answer. Firestore rules already allow participant updates (no rules change).
- **Bug 1** (incoming-call notification): new `sendIncomingCallNotification` in `functions/src/index.ts`.
- **Bug 7** (Android notifications): message-fn android config + host fix; native channels in `src/native/push.ts`; `incoming_call` tap routing.
- **Extras:** foreground toast (`src/components/NativePushToast.tsx` + mounted in `MainLayout.tsx` + dispatch from `push.ts`); home Messages-tile unread badge (`src/hooks/useUnreadMessageCount.ts` + `src/pages/main/home.tsx`); messages-list "Unread" filter + per-chat unread **dots** switched to `lastMessageRead` (`src/pages/messages.tsx`). **All device-confirmed: notifications fire (shade shows content backgrounded; foreground toast), home tile shows count.**
- Everything **type-checks clean** (`npx tsc --noEmit` app + functions, exit 0). NOTE: full `npm run build` / device test happen on Mac (sandbox can't build).

**(4) In progress / half-done**
- **Bug 2 (receiver camera flip)** — DEFERRED to next session. Needs user repro detail: does the receiver's OWN self-view fail to change, or does the CALLER stop seeing the flip? (local-capture vs. sender-replace). `switchCamera()` in `webrtc.ts` reads symmetric in code.
- **Bug 4 (Picture-in-Picture)** — DEFERRED. Needs clarification: OS-level PiP (video pops out of app) vs. the in-app minimized window.
- **Commit/deploy state:** Batch A+B committed & pushed (`origin/main = e236319`: `5dfb1e5`, `44ff11e`, `e236319`). **Batch C + all extras are UNCOMMITTED in git** (deployed from working tree but not yet committed): `functions/src/index.ts`, `src/native/push.ts`, `src/layouts/MainLayout.tsx`, `src/pages/main/home.tsx`, `src/pages/messages.tsx`, + new `src/components/NativePushToast.tsx`, `src/hooks/useUnreadMessageCount.ts`. Functions deployed; hosting + `cap sync` done for earlier extras; the messages-list unread-filter change still needs a redeploy.

**(5) Exact next steps**
1. **Commit + push the uncommitted Batch C + extras** (see file list above) and redeploy the messages-list unread-filter change (`npm run build && firebase deploy --only hosting && npx cap sync android` → reinstall).
2. **Bug 2:** get the camera-flip repro detail, then fix `switchCamera()` (likely the `facingMode:{exact}` constraint failing on the receiver's device, or local-stream reference handling).
3. **Bug 4:** confirm PiP intent, then implement (browser PiP API / Android PiP vs. enhancing the in-app minimized window).
4. **Optional message-count badges:** to show exact unread *message* totals (not conversation count), increment `unreadCount` in `sendNewMessageNotification` and sum it (needs functions redeploy).
5. **Group calls** (`groupCalls/{roomId}`) have NO incoming-call push — separate trigger if wanted. Catering notifications (`notificationRouter.ts`) likely also need the android-channel treatment.
6. Carry-forward from Session 40: shareable APK, production TURN, Play (post business registration).

**(6) Important context & files**
- **`unreadCount` is dead** (never incremented) — use `lastMessageRead` for unread. `lastMessageRead` set false on send (messages.tsx:1351), true on read (802/845).
- **Caller-initiated ICE restart** uses new `calls/{callId}` fields `restartOffer`/`restartAnswer`; callee dedups via `lastHandledRestartSdp`. Recovery only triggers post-disconnect (can't break a healthy call).
- **PiP geometry** is shared between the persistent `<video>` and the controls overlay via `getPipDims()`/`defaultPipPosition()` (module-level in `GlobalCallOverlay.tsx`); frame is transparent for video so the video shows through.
- **Notification channels** `messages`/`calls` (importance 5) must exist on-device — created in `src/native/push.ts` on registration; channelIds must match the Cloud Functions payloads.
- **FCM token IS registering** on the Samsung tablet (confirmed: `users/{uid}.fcmTokens` has a token). `firebase-messaging-sw.js` notification-click host is still hardcoded to `mithr-1e5f4.web.app` (web-only follow-up).
- Files touched this session: `src/components/GlobalCallOverlay.tsx`, `src/utils/webrtc.ts`, `functions/src/index.ts`, `src/native/push.ts`, `src/layouts/MainLayout.tsx`, `src/pages/main/home.tsx`, `src/pages/messages.tsx`; NEW `src/components/NativePushToast.tsx`, `src/hooks/useUnreadMessageCount.ts`.

*Updated May 30, 2026 (Session 41) — Fixed 6 of 8 message-module bugs: PiP minimized window (show video, unblock composer, drag + double-tap via touch/mouse for Android WebView — Bugs 3/6/8), call-drop recovery (grace window + ICE restart — Bug 5), incoming-call FCM Cloud Function (Bug 1), Android notification channels + high-priority payload (Bug 7). Plus foreground push toast, home Messages-tile unread badge, and messages-list unread filter/dots — ALL via `lastMessageRead` because `unreadCount` is never incremented (dead). All device-confirmed working; all type-check clean. DEFERRED to next session: Bug 2 (receiver camera flip — need repro detail) and Bug 4 (PiP — need OS-level-vs-in-app clarification). Batch A/B committed+pushed (origin/main e236319); Batch C + extras still UNCOMMITTED (deployed from working tree) — commit them next: functions/src/index.ts, src/native/push.ts, src/layouts/MainLayout.tsx, src/pages/main/home.tsx, src/pages/messages.tsx, + new NativePushToast.tsx & useUnreadMessageCount.ts. Firebase project is mithr-1e5f4. Builds are Mac-only.*

---

### Session 42 (May 30, 2026) — Finish Calling Bugs (camera flip, self-view, receiver teardown) + Latency + Native Bluetooth Audio + Fluid Responsive UI + Vanishing-Messages/Scroll Fixes

**(1) What we were working on**
Continuing the message-module hardening for the Android launch, driven by real on-device tester feedback: finished the two deferred calling bugs (2, 4), found and fixed a new call-teardown bug (9) + teardown latency, fixed a serious "sender's own message not visible" issue, built **native Android Bluetooth/speaker audio routing**, did a **fluid responsive refactor** of the chat (layout + fonts) so it adapts to any device instead of hard-coded breakpoints, and fixed two more tester-reported messaging bugs (messages vanishing on new message; thread not scrolling to bottom on open). All committed/pushed and type-clean.

**(2) Decisions made & why**
- **Bug 2 (receiver camera flip) — Android can't open two cameras at once.** Root cause: receiver was on **Android browser**; `getUserMedia` for the back camera throws `NotReadableError` while the front is still live (iOS/desktop allow concurrent access — why it worked there). Fix: `switchCamera()` now **stops/releases the current camera BEFORE** requesting the new one, with exact→soft `facingMode` fallback and **restore-the-original-camera** if the flip fails (so a failed flip never leaves dead video). Brief freeze during switch is expected.
- **Bug 4 (PiP "image gone on maximize") — it's the LOCAL self-view, not the remote.** Confirmed via the user's video (remote stayed because it's a persistent element). The self-view `<video>` only renders in fullscreen, so it **unmounts in PiP and remounts on expand**; the attach effect only re-ran on stream change, so the remounted element kept a null `srcObject`. Fix: the local-video effect now also depends on `callMinimized` → re-attaches on expand.
- **Bug 9 (receiver call not torn down when caller ends).** `listenForIncomingCalls` only handled `added` docChanges — nothing watched the call doc while RINGING, so caller hang-ups never cleared the receiver (across all open sessions). Fix: handle `modified`/`removed` too — end on terminal status (ended/rejected/missed) and clear the ringing UI when the call is **answered on another device** (status→connected while still ringing, without ending the live call).
- **Teardown latency.** `endCall` **awaited** the Firestore status write before tearing down (delayed both ends), plus a **2000ms "Call ended"** screen. Fix: fire the end-signal **non-blocking** (peer notified ASAP) and shorten the ended screen to **700ms**. Remaining latency is inherent Firestore propagation (~0.5–1.5s); the Bug-5 grace window (12–15s) only applies to genuine signal loss, not normal hang-ups.
- **Sender's own message not visible until next message.** Root cause: list orders by `createdAt` (a `serverTimestamp()`), which is **null locally** until the server resolves it → the just-sent message had no timestamp for ordering/date-grouping and didn't render. Fix: read with `d.data({ serverTimestamps: 'estimate' })` so pending writes get a local estimate immediately. (Chose this minimal fix over optimistic-insert.)
- **Native Bluetooth/speaker audio routing.** The WebView can't route call audio (`setSinkId` unsupported in Android System WebView), so built a native Capacitor plugin driving `AudioManager`. Scope chosen: **auto-route to a connected Bluetooth/car device + a Speaker toggle** (not a full picker). **Defensive** — every AudioManager call is try/catch and the JS bridge no-ops on web, so it **can't break working call audio**. Added **runtime `BLUETOOTH_CONNECT`** request (Android 12+) so it's fully automatic. Two-way calls need **HFP** devices (A2DP speakers are output-only).
- **Fluid responsive UI (tester saw incoming bubbles cut off / cramped on a narrower device).** Decided to move from **stepwise breakpoints** (`px-3 sm:px-[5%]`, `max-w-[85%] sm:max-w-[65%]`, fixed `text-[Xpx]`) to **fluid `clamp()`/`min()`** that scale continuously with the device — "dynamic per device, no hard-coding." Preserved the working tablet feel (≈5% mid-range), floored small screens. Used **inline styles / CSS classes** for clamp (avoids Tailwind arbitrary-value parsing quirks with commas).
- **Vanishing messages — it's an async-handler RACE, not decryption.** Every message IS pushed to the list (failed decryption shows a 🔒 placeholder, never dropped). The message listener is `async` (awaits decryption), so a slower/older snapshot handler's `setMessages` could land AFTER a newer one, overwriting with a stale subset. Fix: a **monotonic sequence guard** (`msgSnapshotSeqRef`) — only the latest snapshot's result is applied.
- **Scroll-to-bottom unreliable on open.** Was smooth `scrollIntoView` fired once at 100ms (before layout/images settle). Fix: pin the container directly (`scrollTop = scrollHeight`), and on a thread's first load retry at render+200ms+500ms.

**(3) Completed (all committed + pushed; origin/main = `479b42d`; all type-check clean)**
- `src/utils/webrtc.ts`: Bug 2 (`switchCamera` stop-first + `attachNewVideoTrack` helper), Bug 9 (`listenForIncomingCalls` handles modified/removed), teardown latency (non-blocking end signal + 700ms ended screen).
- `src/components/GlobalCallOverlay.tsx`: Bug 4 (self-view reattach on `callMinimized`), Speaker button + call-audio lifecycle wiring.
- `src/pages/messages.tsx`: sender-message fix (`serverTimestamps:'estimate'`), fluid padding (`clamp(16px,5%,56px)`) + fluid bubble width (`min(85%,480px)`), `.chat-text`/`.chat-meta` applied, **race guard** + **reliable scroll**.
- `src/index.css`: `.chat-text` / `.chat-meta` fluid font classes.
- `src/components/messages/LinkPreviewCard.tsx`: fluid typography (chat-meta + clamp).
- **Native audio (needs full Android rebuild):** NEW `android/app/src/main/java/enovoapp/app/CallAudioPlugin.java` (`AudioManager`: MODE_IN_COMMUNICATION, BT SCO auto-route, speaker, runtime BLUETOOTH_CONNECT); `MainActivity.java` registers it; `AndroidManifest.xml` adds `BLUETOOTH_CONNECT` (+ legacy `BLUETOOTH` maxSdk 30); NEW `src/native/callAudio.ts` (guarded JS bridge).
- Commit history: `479b42d` (vanishing/scroll), `c4cd80a` (LinkPreviewCard fluid), `eb6ffc5` (fluid fonts), `489e840` (fluid layout), `af6f0e5` (native audio + BLUETOOTH_CONNECT), `46c2319` (sender-message). Plus earlier `92e3b6c` (Bug 2/4), `f118d48` (Bug 9), `e236319`/`44ff11e`/`5dfb1e5` (Session 41 batches).

**(4) In progress / half-done (need DEPLOY + on-device verification)**
- Everything is committed but **device verification pending**. Bluetooth needs a **full Android rebuild** (`Run ▶`/gradle, not just `cap sync`) + a real BT/car device test. Fluid UI needs a phone-vs-tablet check. Vanishing-messages + scroll need the tester's Android app re-test.
- **Bluetooth UX choice open:** the `BLUETOOTH_CONNECT` prompt currently appears on the **first call** (point of use). Could move it up-front (e.g., after sign-in) if interrupting a call is undesirable.
- **Fluid pattern only applied to the chat surface** (layout + fonts + link previews). Other screens (feed, business, profile, etc.) still use stepwise breakpoints — extend incrementally if wanted.
- If vanishing messages persist after the race-guard deploy, next step is a `chrome://inspect` console look on the tester's device.

**(5) Exact next steps**
1. **Deploy everything**: `npm run build && firebase deploy --only hosting` (web) + `npx cap sync android` + **REBUILD in Android Studio** (for the native `CallAudioPlugin`) + reinstall. (Functions unchanged this session — no `--only functions` needed.)
2. **Device-test**: Bluetooth/car auto-route + speaker toggle (real BT device, accept the nearby-devices prompt on first call); fluid chat on phone + tablet; vanishing-messages + scroll-to-bottom in the tester's Android app.
3. Optional polish: move BT permission prompt up-front; add a "near-bottom" guard so new-message auto-scroll doesn't yank users reading history; extend fluid `clamp()`/`min()` to other screens.
4. Carry-forward (still open from Sessions 40–41): build a **shareable APK** (`./gradlew assembleRelease`); provision a **production TURN** (free openrelay drops cellular calls; `VITE_TURN_*` wiring ready in `iceConfig.ts`); **Play Store** path post **business registration** (Organization account skips the 12-tester/14-day rule) + add the **Play App Signing SHA-256** to `assetlinks.json`; fix `firebase-messaging-sw.js` notification-click host (still `mithr-1e5f4.web.app`). Web push VAPID key is already set in `.env`.

**(6) Important context, constraints & files**
- **All recent work is committed AND pushed — origin/main = `479b42d`.** The Cowork workspace **auto-commits + pushes**, so on the Mac a manual `git add`/`commit` often reports "nothing to commit" / "branch up to date" — that means it already landed; just `git pull` if your Mac is behind, then deploy. The `.firebase/hosting.ZGlzdA.cache` modified file is a deploy artifact — don't commit it.
- **Three deploy types:** (a) **client** (React/CSS/TS) → `npm run build` + `firebase deploy --only hosting` + `cap sync` + reinstall; (b) **server** (Cloud Functions, `functions/`) → `firebase deploy --only functions`; (c) **native** (Android Java — the audio plugin, MainActivity, manifest) → full Android rebuild in Android Studio. Most of this session was client; the Bluetooth plugin is native.
- **Builds are Mac-only** (Rollup/Gradle); the Cowork sandbox can read/grep/edit + run `tsc --noEmit` but NOT `npm run build`/Gradle.
- **Key technical facts (carry-forward):** `unreadCount` is dead — use `lastMessageRead`. Pending `serverTimestamp` reads null unless `d.data({serverTimestamps:'estimate'})`. The message listener is `async`; out-of-order results are guarded by `msgSnapshotSeqRef`. `switchCamera` must stop the current camera before opening the next (Android). The local self-view remounts on PiP expand → its attach effect depends on `callMinimized`. Call teardown end-signal is fire-and-forget; "Call ended" screen is 700ms. `CallAudio` native plugin is best-effort (try/catch) and no-ops on web. Fluid chat sizing lives in inline `clamp()`/`min()` styles + `.chat-text`/`.chat-meta` in `index.css`.
- Files NEW this session: `android/app/src/main/java/enovoapp/app/CallAudioPlugin.java`, `src/native/callAudio.ts`. EDITED: `src/utils/webrtc.ts`, `src/components/GlobalCallOverlay.tsx`, `src/pages/messages.tsx`, `src/index.css`, `src/components/messages/LinkPreviewCard.tsx`, `android/app/src/main/java/enovoapp/app/MainActivity.java`, `android/app/src/main/AndroidManifest.xml`.

**Tech stack (carry-forward):** React 19.2 + TypeScript 5.9 (strict) + Vite 7 + `vite-plugin-pwa`; **Tailwind v4 (CSS-first, NO `tailwind.config.ts`)**; Firebase Auth (email/password + phone via reCAPTCHA — **no social OAuth**), Firestore, Storage, Cloud Functions, FCM; **Capacitor 7** for Android (now with a custom native `CallAudio` plugin); react-router-dom 7, i18next, framer-motion, recharts, crypto-js (E2E messages), zod. AGP 8.13.2 / Gradle 8.13.

**Design system / brand / tone (carry-forward):**
- Brand **eNoVo** (mixed-case), tagline **"Our culture, connected."**, domain `enovoapp.com`. User-facing label for the ethnicity concept is **"Heritage."** Tone: warm, community-first, diaspora-focused.
- **Aurora palette** (source of truth `src/constants/theme.ts`): primary **Electric Indigo `#6366F1`** (light `#818CF8`, dark `#4F46E5`), secondary **Soft Mint `#10B981`**, tertiary **Aurora Cyan `#22D3EE`**, gradient indigo→mint. Light bg `#F5F6FA` / surface `#FFFFFF`; dark bg `#1A1B2E` / surface `#232438`. Text `#1E2132`/`#5B5E72`/`#9295A5`. Semantic: success `#10B981`, danger `#EF4444`, warning `#F59E0B`. **Logo artwork sits on navy `#091827`** (app icon/splash bg).
- **Responsive philosophy (NEW this session):** prefer **fluid sizing** (`clamp()`, `min()`, `%`, `rem`, `dvh`) over hard-coded breakpoints so the UI scales continuously to any device. Chat is converted; roll out to other screens incrementally with min/max bounds that preserve the tablet baseline.
- CSS vars keep `--color-sangam-*` prefix (intentional, `src/index.css`); localStorage/IndexedDB keys keep `sangam_*`/`ethnicity_*` prefixes (data preservation). PWA `theme_color`/`background_color` `#F5F6FA`.

*Updated May 30, 2026 (Session 42) — Closed the last 2 deferred calling bugs (Bug 2 Android camera-flip via stop-first; Bug 4 PiP self-view reattach on expand) + new Bug 9 (receiver teardown via modified/removed docChanges) + teardown latency (non-blocking end signal, 700ms ended screen) + sender's-own-message visibility (serverTimestamps:'estimate'). Built NATIVE Android Bluetooth/speaker audio routing (CallAudioPlugin.java + runtime BLUETOOTH_CONNECT + Speaker toggle, defensive/no-op-on-web). Fluid-responsive refactor of the chat (clamp()/min() padding, bubble width, fonts; LinkPreviewCard too) replacing stepwise breakpoints. Fixed vanishing-messages (async snapshot race guard `msgSnapshotSeqRef`) and unreliable scroll-to-bottom on open (pin container + retries). ALL committed+pushed (origin/main `479b42d`), all type-check clean. NEXT: deploy (client hosting + cap sync + FULL Android rebuild for the native plugin + reinstall) then device-test Bluetooth/fluid/vanishing/scroll; then carry-forward APK/TURN/Play. Firebase project mithr-1e5f4; builds Mac-only; workspace auto-commits+pushes.*

---

### Session 43 (May 30, 2026) — Cross-Browser Audit + Touch/Desktop Patches + Stale-Chunk Auto-Recovery

**(1) What we were working on**
Making sure the Session 41–42 fixes (the 9 calling/messaging bugs + UI work) aren't Android-only — verifying they hold on **desktop Chrome/Safari/Firefox, iOS Safari, and Android Chrome** — and patching the concrete cross-browser issues found. Then diagnosing and fixing a tester-reported **"Something went wrong"** error-boundary screen on the Android app.

**(2) Decisions made & why**
- **Cross-browser stance:** the fixes were already largely portable by design — Bug 8 PiP gestures use **touch + mouse** (not Pointer Events) so they cover iOS Safari/Android (touch) and desktop (mouse); Bug 5 handles **both** `connectionState` and `iceConnectionState` (Safari only reliably fires the latter) and uses `createOffer({iceRestart:true})`; Bug 4 uses `safariSafePlay` + `playsInline`; fluid CSS uses `clamp()`/`min()`/`dvh` (with the `@supports`→`vh` fallback). Native Bluetooth audio **no-ops on web** (OS handles routing there). So only two real gaps needed patching:
  - **Synthetic-mouse double-tap (iOS Safari + Android Chrome):** touch devices fire a *synthesized* mouse event ~300ms after a tap, which re-ran the PiP tap/double-tap logic → a single tap could be misread as a double-tap and falsely expand. Fix: a `lastPipTouchRef` timestamp; `handlePipMouseDown` ignores mouse events within 700ms of a touch. Desktop unaffected.
  - **Camera flip on single-camera devices (desktop):** flip would stop+restart the only camera (flicker/freeze). Fix: `enumerateDevices` guard in `switchCamera()` — skip flip when <2 cameras. Mobile front/back unaffected.
- **"Something went wrong" error screen = stale-chunk / service-worker cache, NOT a code bug.** Confirmed by "reload fixes it." After a new build, hashed chunk filenames change; a cached SW serving the old `index.html` requests chunks that no longer exist → a lazy screen fails → the `AppErrorBoundary` shows the fallback. Decision: **self-heal** rather than chase a phantom bug — added a `vite:preloadError` listener in `main.tsx` that reloads **once** (loop-guarded via `sessionStorage`) to pull the fresh build, so users never see the error. (Chose the safe auto-recovery over disabling the SW on native, which would be cleaner but riskier — left as an optional follow-up.)
- **Install hygiene learning:** an Android Studio **Run ▶ is an *incremental* install** — it keeps the existing WebView data, **including the stale SW cache**, which is what triggered the error. A **full uninstall+reinstall** (or Clear data) is needed after a big update to take the new SW cleanly.

**(3) Completed (all type-check clean)**
- `src/components/GlobalCallOverlay.tsx`: `lastPipTouchRef` + 700ms guard in `handlePipMouseDown` so synthesized mouse-after-touch can't double-fire the PiP gesture.
- `src/utils/webrtc.ts`: `enumerateDevices` single-camera guard at the top of `switchCamera()`.
- `src/main.tsx`: `vite:preloadError` → one-time guarded `window.location.reload()` to self-heal stale-chunk errors.
- Full cross-browser audit documented (each fix's portability confirmed at the code level).

**(4) In progress / half-done**
- **Device verification still pending** across browsers — this was a *code-level* audit (no browser farm here). Smoke-test the calling flow on desktop Safari + Firefox, iOS Safari (PWA), Android Chrome — especially PiP drag/double-tap (a single tap must NOT expand) and camera flip (no flicker on desktop).
- **Stale-chunk root cause not eliminated, only recovered.** Optional stronger fix: **skip the precache service worker on the native Capacitor build** (it loads assets locally, so the SW only adds stale-cache risk) while keeping it for the web PWA — prevents even the brief auto-reload flash. Not yet done.
- All Session 42 device tests (Bluetooth/fluid/vanishing/scroll) still pending on-device.

**(5) Exact next steps**
1. On the tablet, do a **clean install** of the latest build: `git pull` → `npm run build` → `npx cap sync android` → **uninstall eNoVo first (or Clear data)** → install via Android Studio. (Run ▶ alone keeps the stale cache.)
2. Deploy web: `firebase deploy --only hosting`. (Client-only this session — no functions/native rebuild needed for the cross-browser + chunk patches; but the Session-42 **native Bluetooth plugin still needs a full Android rebuild**.)
3. Cross-browser smoke test (above).
4. Optional: skip precache SW on native (eliminate stale cache entirely); device-test Bluetooth/fluid/vanishing/scroll from Session 42.
5. Carry-forward (Sessions 40–42): shareable APK; production TURN (`VITE_TURN_*` ready in `iceConfig.ts`); Play Store post business-registration (Org account skips 12-tester/14-day rule) + add Play App Signing SHA-256 to `assetlinks.json`; fix `firebase-messaging-sw.js` notification-click host (`mithr-1e5f4.web.app` → `enovoapp.com`).

**(6) Important context, constraints & files**
- **The "Something went wrong" screen is the `AppErrorBoundary`** (App.tsx) catching a render/chunk error; "fixes on reload" ⇒ almost always a **stale-chunk/SW** issue, now auto-recovered by the `vite:preloadError` handler in `main.tsx`. A genuine code crash would show a stack into our files via `chrome://inspect` Console instead.
- **Android Studio Run ▶ = incremental install** (keeps WebView/SW cache). After a significant update, **uninstall+reinstall or Clear data** for a clean state.
- **Touch devices synthesize mouse events** after taps — any custom touch+mouse gesture handler must guard the mouse path against post-touch synthetics (pattern: `lastTouchRef` + time window). Applied in `GlobalCallOverlay`.
- **`switchCamera` skips when <2 cameras** (desktop). Files this session: EDITED `src/components/GlobalCallOverlay.tsx`, `src/utils/webrtc.ts`, `src/main.tsx`.
- Deploy types unchanged: **client** (build+hosting+cap sync), **server** (functions), **native** (Android rebuild). This session was client-only.
- Workspace **auto-commits + pushes** — manual `git add`/`commit` on the Mac often says "nothing to commit / up to date"; that means it already landed (`git pull` if the Mac is behind). `.firebase/hosting.ZGlzdA.cache` is a deploy artifact — don't commit it.

**Tech stack (carry-forward):** React 19.2 + TypeScript 5.9 (strict) + Vite 7 + `vite-plugin-pwa`; **Tailwind v4 (CSS-first, NO `tailwind.config.ts`)**; Firebase Auth (email/password + phone reCAPTCHA — **no social OAuth**), Firestore, Storage, Cloud Functions, FCM; **Capacitor 7** for Android (custom native `CallAudio` plugin); react-router-dom 7, i18next, framer-motion, recharts, crypto-js (E2E messages), zod. AGP 8.13.2 / Gradle 8.13.

**Design system / brand / tone (carry-forward):**
- Brand **eNoVo** (mixed-case), tagline **"Our culture, connected."**, domain `enovoapp.com`. User-facing label for the ethnicity concept is **"Heritage."** Tone: warm, community-first, diaspora-focused.
- **Aurora palette** (`src/constants/theme.ts`): primary **Electric Indigo `#6366F1`** (light `#818CF8`, dark `#4F46E5`), secondary **Soft Mint `#10B981`**, tertiary **Aurora Cyan `#22D3EE`**, gradient indigo→mint. Light bg `#F5F6FA` / surface `#FFFFFF`; dark bg `#1A1B2E` / surface `#232438`. Text `#1E2132`/`#5B5E72`/`#9295A5`. Semantic: success `#10B981`, danger `#EF4444`, warning `#F59E0B`. **Logo on navy `#091827`** (icon/splash bg).
- **Responsive philosophy:** prefer **fluid sizing** (`clamp()`, `min()`, `%`, `rem`, `dvh`) over hard-coded breakpoints; chat is converted, roll out to other screens incrementally with min/max bounds that preserve the tablet baseline.
- **Cross-browser philosophy (NEW):** target desktop Chrome/Safari/Firefox + iOS Safari + Android Chrome; use touch+mouse (not Pointer Events) for custom gestures and guard synthesized mouse-after-touch; handle both WebRTC connection-state events (Safari); keep `dvh`/`clamp` fallbacks.
- CSS vars keep `--color-sangam-*` prefix (intentional, `src/index.css`); localStorage/IndexedDB keys keep `sangam_*`/`ethnicity_*` prefixes (data preservation). PWA `theme_color`/`background_color` `#F5F6FA`.

*Updated May 30, 2026 (Session 43) — Cross-browser audit of the 9 fixes (desktop Chrome/Safari/Firefox, iOS Safari, Android Chrome): most were already portable (touch+mouse gestures, dual WebRTC state events, safariSafePlay, clamp/min/dvh, web no-op for native audio). Patched 2 real issues: PiP synthetic-mouse-after-touch double-tap (iOS Safari/Android) and camera flip on single-camera desktops (enumerateDevices guard). Diagnosed the tester "Something went wrong" screen as a STALE-CHUNK / service-worker cache issue (reload fixes it), and added `vite:preloadError` auto-reload (loop-guarded) in `main.tsx` to self-heal. Learned: Android Studio Run ▶ is incremental and keeps the stale SW cache — do a full uninstall+reinstall after big updates. All client-only, type-check clean (commit on the Mac: `git add src/components/GlobalCallOverlay.tsx src/utils/webrtc.ts src/main.tsx`). NEXT: clean reinstall of latest build + cross-browser smoke test; optional skip-SW-on-native; then Session-42 device tests + carry-forward APK/TURN/Play. Firebase mithr-1e5f4; builds Mac-only; workspace auto-commits+pushes.*

---

### Session 44 (July 2, 2026) — Full-App Code Review + Phase A Remediation (Aggregation Queries, reportError, Per-Route Boundaries, ToastContext)

**What was done:**
1. **Full-app production code review** — 4 parallel audit passes (memory/perf, architecture, stability, UI/UX) + manual verification of every high-severity claim. Report: `../eNoVo_Code_Review_07_02_2026.md`. Phased remediation plan: `../eNoVo_Implementation_Plan_07_02_2026.md` (Phases A–D covering all 10 roadmap items).
2. **A1 — Admin dashboard aggregation** — NEW `src/services/adminStats.ts`: `getDashboardStats()` uses `getCountFromServer()` for all 16 dashboard counts (was: downloading 15 ENTIRE collections in parallel just to read `.size`). The 7-day signup histogram is now a bounded range query on `users.createdAt >= now-7d`. `admin.tsx` `loadDashboardData()` reduced from ~66 lines to a service call; failure now shows toast.
3. **A2 — Forum topic counts** — `forum.tsx` `loadTopics()` uses per-topic `getCountFromServer` (total − isRemoved==true, preserving legacy-doc semantics where isRemoved is missing; a `==false` filter would skip them). Equality-only queries → NO composite index needed (index merging). Added `getCountFromServer` to firestore imports.
4. **A3 — Deleted the feed.tsx client-side migration** — the 'social'→'community' effect ran a FULL posts-collection scan per device (localStorage guard) forever. Removed; comment points at `migrate-social-to-community.cjs` for stragglers.
5. **A4 — NEW `src/utils/reportError.ts`** — central error reporter (op tag + console + optional toast). Policy: `.catch(() => {})` allowed ONLY for pure telemetry (businessAnalytics). Wired: catering.tsx vendor-notify paths (4 sites — quote edited / submitted / vendors bell / RFQ multichannel, plus owner-resolution catch), messages.tsx get-active-group-call + set-typing-status.
6. **A5 — Per-route error boundaries** — `App.tsx`: `AppErrorBoundary` now takes optional `scope` prop → compact in-shell fallback ("Try Again" resets state / "Reload App"); new `Page` wrapper (boundary + Suspense) applied to all 18 private routes. A crash in one module no longer unmounts the whole app; root boundary unchanged for auth routes.
7. **A6 — ToastContext fix** — timer-per-toast created at add-time in a `timersRef` Map (was: useEffect([toasts]) tearing down + recreating ALL timers on every toast change, restarting durations mid-flight); provider `value` now `useMemo`'d (was the only non-memoized context).

**Verified:** `./node_modules/.bin/tsc --noEmit -p tsconfig.app.json` clean. All `--aurora-*` vars used by the new fallback UI confirmed to exist in index.css.

**NOT committed** — commit from Mac:
```bash
git add src/services/adminStats.ts src/utils/reportError.ts src/App.tsx src/contexts/ToastContext.tsx src/pages/admin.tsx src/pages/forum.tsx src/pages/feed.tsx src/pages/catering.tsx src/pages/messages.tsx hand_off_note_04_16.md
git commit -m "perf+stability: aggregation counts, reportError, per-route boundaries, toast timers (Session 44 / review Phase A)"
```
Then build + deploy (hosting only; no index/rules changes needed): `./node_modules/.bin/tsc -b && ./node_modules/.bin/vite build && firebase deploy --only hosting`.

**Test checklist after deploy:** (1) admin dashboard — all count cards + weekly signup chart match previous values; (2) forum landing — topic thread counts correct incl. topics with removed threads; (3) feed loads normally; (4) submit + edit an RFP — success toasts appear, no console errors; (5) toasts auto-dismiss on time with several stacked; (6) force a crash in one page (temp throw) → inline "This section hit an error" card, nav still works.

**Next (Phase B, from `../eNoVo_Implementation_Plan_07_02_2026.md`):** shared `<Modal>` (components/ui/Modal.tsx wrapping useModalA11y) + migrate events/housing/forum/marketplace modals; replace 53 alert()/confirm(); skeletons on feed/forum/housing; aspect-ratio on card images. Phase C: services/feed.ts + services/forum.ts with zod + limit(); shared useModeration; profile.tsx `as any` burn-down (27). Phase D: messages.tsx decomposition (76 useState), media → Firebase Storage (decide vs. `../eNoVo_Database_Options_Analysis.md` first), E2EE + reprice-math tests. Carry-forward from S40-43: shareable APK, production TURN, Play submission post business registration, www.enovoapp.com domain entry, Session-42 device tests.

*Updated July 2, 2026 (Session 44) — Full-app code review (report + phased plan saved next to project) and Phase A remediation implemented: admin dashboard + forum counts moved to server-side aggregation (getCountFromServer — was downloading 15 full collections per admin load and all forumThreads per forum visit), feed.tsx per-device full-scan migration deleted, central reportError utility replacing silent .catch(() => {}) on vendor-notification paths, per-route error boundaries (Page wrapper, scope'd fallback keeps app shell alive), ToastContext timer-per-toast + memoized value. tsc clean; NOT yet committed/deployed — commit + build + deploy from Mac, then run the 6-point test checklist. Firebase mithr-1e5f4; builds Mac-only.*

---

### Session 45 (July 2, 2026) — Review Phase B: Shared Modal + A11y Migration (4 pages), alert() Purge, Lazy Images

**What was done (continues Session 44's review remediation; plan: `../eNoVo_Implementation_Plan_07_02_2026.md`):**
1. **NEW `src/components/ui/Modal.tsx`** — shared modal shell wrapping `hooks/useModalA11y`: ESC + focus trap + focus restore + body scroll lock + backdrop-click close + ARIA dialog + mobile bottom-sheet / desktop card (Aurora, dark mode). Props: `open, onClose, title, size sm|md|lg|xl, layer base(z-50)|high(z-[70]), hideHeader, disableBackdropClose, panelClassName`. Z-policy matches business module.
2. **NEW `src/hooks/useBodyScrollLock.ts`** — for bespoke overlays/takeovers not using the shell.
3. **18 bespoke modals migrated** to the shell across 4 pages, STRICT parity (every field/button/handler preserved; only backdrop/panel scaffolding replaced): events (delete/report/block), housing (delete-listing, delete-comment, report, block), forum (delete-thread, delete-reply, content-warning, report, block, create-thread), marketplace (create/edit, detail, delete, report, block). All previously had NO focus trap; most had no ESC.
4. **DELIBERATELY NOT migrated (parity rule):** full-screen takeovers (events create/edit wizards, housing create/edit, housing photo-gallery lightbox) and the events/housing detail modals (complex pinned-hero/pinned-footer layouts) — these keep their exact layout and instead got `useBodyScrollLock` coverage (events.tsx ~line 500, housing.tsx replaces its bespoke overflow effect with a superset). Transparent context-menu overlays untouched.
5. **alert() purge:** 13 replaced (11 marketplace, 2 housing) with each page's existing `setToastMessage` toast — message text byte-identical. events/forum had none. 40 alert()/confirm() remain in OTHER files (profile, settings, admin, etc.) — future sweep.
6. **Lazy images:** added `loading="lazy" decoding="async"` to 3 card images missing it (events featured carousel, housing card hero, marketplace list thumbnail).
7. **Review corrections (stale findings):** feed/forum/housing ALREADY have skeleton loaders, and card images sit in fixed-height containers (no CLS risk) — those review items were satisfied by prior sessions; no work needed.

**Known intentional deltas (shell-policy, accepted):** migrated dialogs become bottom-sheets on mobile; backdrop standardized to `bg-black/40 backdrop-blur-sm`; report/block panels now `bg-aurora-surface` (dark-mode aware); stacked dialogs z-[60]→z-[70]; confirm dialogs gained backdrop-click + ESC close. Pre-existing quirk noted: housing/events page-level ESC listener closes the detail modal even when a stacked shell-Modal is also closing — same net behavior as before, review in Phase D.

**Verified:** `./node_modules/.bin/tsc --noEmit -p tsconfig.app.json` exit 0. Parity greps: all delete/report/block handlers present; 0 `alert(` left in the 4 pages; Modal usage counts events 3 / housing 4 / forum 6 / marketplace 5.

**NOT committed — commit from Mac:**
```bash
git add src/components/ui/Modal.tsx src/hooks/useBodyScrollLock.ts src/pages/events.tsx src/pages/housing.tsx src/pages/forum.tsx src/pages/marketplace.tsx hand_off_note_04_16.md
git commit -m "ux+a11y: shared Modal shell, 18 modal migrations, alert() purge, lazy images (Session 45 / review Phase B)"
```
Build + deploy (hosting only): `./node_modules/.bin/tsc -b && ./node_modules/.bin/vite build && firebase deploy --only hosting`. NOTE: Session 44 changes may still be uncommitted too — commit both together or in order.

**Test checklist after deploy (per page: events, housing, forum, marketplace):** (1) open each migrated dialog — content/buttons identical to before; (2) ESC closes it, Tab cycles inside it, focus returns to trigger on close; (3) backdrop click closes; (4) mobile: dialogs slide as bottom sheets; (5) page behind doesn't scroll while any modal/takeover is open; (6) marketplace: trigger validation errors on create form → toast pill (not browser alert); submit report/block failures → toast; (7) full-screen create/edit wizards look and behave EXACTLY as before; (8) forum create-thread: title/content counters, flair picker, content-warning stacking on top; (9) dark mode pass on all migrated dialogs.
...
**Session 51 addendum — ETA validation re-wired + D1 tranche 2 (moderation domain):**
1. **validateDeliveryETA is BACK IN PRODUCTION USE** (was uncalled since S37): OrderCard.tsx gained `validateTimeEta()` — per product decision, a 'time'-mode ETA means that clock time ON THE ORDER'S EVENT DATE (normalizes Timestamp|Date|string eventDate → local Y-M-D, composes with HH:MM, validates future + ≤ event EOD); 'duration'-mode skips validation (relative to now). Wired into BOTH sites: prep-time Share button and Dispatch-for-Delivery — invalid → error toast, action aborted (vendor can clear the field to proceed without an ETA on dispatch).
2. **D1 tranche 2 DONE:** NEW `hooks/useChatModeration.ts` — reducer for the 9 report/block modal states (OPEN_REPORT sets 6 fields atomically) + plain useState for blockedUsers (preserves the page's functional-setter shape). messages.tsx 73→63 useState; Firestore I/O deliberately stayed in the page (moves with future services/messages.ts). Two unobservable semantics merges documented in the agent report (close now always resets the form; blockTargetUser nulled on cancel — JSX guards make both invisible). Plan doc updated (domain 2 ✅, next = domain 3 context-menu/delete).
**MANUAL LIVE TESTS REQUIRED before tranche 3 (per plan rules):** (a) Messages: report a message end-to-end (fresh form, category-gated submit, success toast, reports+modQueue docs, second report increments count), all three cancel paths, block from context menu AND chat header (correct name, arrayUnion write, conversation disappears + stays hidden after reload); (b) Catering vendor: prep-time Share with a past time on a today-event order → error toast; valid time → shares; dispatch with invalid/valid/empty ETA; duration mode unaffected.
...
**Session 52 addendum 2 — Review coverage audit (grep-verified, not from memory):** `../eNoVo_Review_Coverage_Audit_07_03_2026.md` audits every code-review finding against deployed code. Scoreboard: roadmap 6/10 fully done (several exceeded), 2 partial with named gaps, 1 in-progress by design (messages), 1 deferred by user decision (media→Storage). VERIFIED GAPS now tracked: (1) 30 bare `.catch(() => {})` in 13 files outside analytics (webrtc/call overlays/catering components) — policy sweep never done; (2) groupWebrtc peer-stream cleanup never verified (only §2 critical sub-item not investigated); (3) admin SECTION loaders still fetch 7 full collections (dashboard fixed, lists not paginated); (4) dead pages/main auth duplicates still present; (5) 4 duplicated hidden-notification helpers (S48-created); (6) 31 arbitrary-hex classes untouched; (7) inline error states (feed/events/housing) never added; (8) global z-scale not established; (9) `: any` 365→359 / `as any` 158→129 — global burn-down + types/firestore.ts reconciliation queued. None of these block anything; all prioritized in the audit doc §6.

---

### Session 53 (July 3, 2026) — Coverage-Audit Gap Cleanup (gaps 1, 2, 4, 5; gap 3 deferred with rationale)

Closes the actionable gaps from `../eNoVo_Review_Coverage_Audit_07_03_2026.md` §6:
1. **Gap 1 — `.catch(() => {})` sweep COMPLETE:** 30 sites across 11 files triaged. 19 now route through `reportError` (17 console-only; 1 toast added in OrderCard prep-ETA where an unconditional success toast was lying to vendors on failure; call paths deliberately toast-free). 12 remain bare, EVERY one with a justification comment ≤2 lines above (autoplay-policy play()/resume() suppressions in webrtc + call overlays, navigator.share AbortError, AudioContext.close race, 3 businessAnalytics telemetry counters). Policy is now fully enforced repo-wide.
2. **Gap 2 — groupWebrtc cleanup VERIFIED, NO LEAK, NO CHANGE:** review §2.7's concern disproven by inspection — `removePeer()` closes the pc + deletes from `peerConnections`, and public `state.peers` is REBUILT from that map on every emit, so departed peers' MediaStreams leave state immediately; `cleanup()` stops local/screen tracks, unsubscribes everything, clears the timer. Per call-path conservatism: documented, untouched.
3. **Gap 4 — dead `pages/main/` duplicates DELETED:** `auth/login.tsx`, `signup.tsx`, `select-ethnicity.tsx` (1,804 lines, zero references, contained the last 8 phantom alert() calls). `pages/main/home.tsx` remains (live, routed).
4. **Gap 5 — hidden-notification helpers UNIFIED:** canonical `sendContentHiddenNotificationDoc()` added to services/moderation.ts; the four module-typed wrappers (feed/events/housing/marketplace) now delegate to it — exactly ONE `addDoc(notifications)` writer for content_hidden remains (verified).
5. **Gap 3 — admin section-list pagination DEFERRED (deliberate):** admin search currently operates over the FULL user/listing sets client-side; naive pagination breaks admin search without server-side search design. Revisit trigger: >1,000 users or admin page feeling slow. Admin-only exposure.

**Verified:** tsc exit 0; 12 bare catches all justified; 1 content_hidden writer; pages/main contains only home.tsx.
**Commit from Mac:** `git add src/ hand_off_note_04_16.md && git commit -m "chore: close review coverage gaps — catch sweep, webrtc verification, dead code, notification dedup (Session 53)"` — then `firebase deploy --only hosting` (predeploy gate runs the 270 tests automatically now) + cap:sync for Android.
...
**Session 54 addendum — Android verified + sync-gap discovered and closed:** User confirmed the photo-picker fix live on the Android tablet (Choose Photo → gallery works). Along the way discovered the tablet had been running a POST-SESSION-49 bundle: Sessions 50–53 code (reprice guards, catch sweep, messages tranches 2–3) never reached Android until today's cap sync — web deploys and Android syncs are separate steps and the copy was skipped for four sessions. Fingerprint audit (S44/S49/S51/S53/S54 markers) confirms the Android bundle is now byte-identical to dist with ALL sessions present. WORKFLOW RULE going forward: ship instructions must always state "web + Android" or "server-only", and Android always ships via `npm run cap:sync` (test+build+copy in one) — never a bare vite build + manual sync. STILL PENDING on-device: the Messages tranche 2/3 live-test checklist (report end-to-end, block flow, context menu + delete confirm) — first device exposure of those refactors is this install. iPad Safari (the original bug reporter's device) gets the fix from the hosting deploy on next visit/refresh — worth a quick Choose Photo check there too.

**Session 55 preamble — DB decision TABLED (July 3):** User reviewed the post-remediation update (§7 of ../eNoVo_Database_Options_Analysis.md) and tabled the decision — no driver selected, no revisit trigger set. Consequences: D2 (media→Storage) remains parked; messages service extraction (domain 10) proceeds Firestore-native when reached; hybrid-slice option stays documented and cheap thanks to the service layer. Revisit when a driver crystallizes (catering search requirements, cost signal, or lock-in strategy call).

---

### Session 55 (July 4, 2026) — Polish Batch: Hex Tokenization + Z-Scale Policy + Inline Error States (review 4.1/4.2/4.6 closed)

DB decision remains TABLED per user (needs more time/review — see preamble above). This session took the polish batch instead (deliberately avoids messages.tsx while its tranche 2/3 device tests are pending):

1. **Arbitrary-hex burn-down (finding 4.1):** audit showed all 31 sites were in ADMIN files. 3 were true aurora-token matches (#6366F1 → `aurora-indigo`) — replaced. The other 28+ were the admin section's own brand-red family (#FF3008/#FF6034/#E02A06) with NO existing token — so the family was TOKENIZED: new `--color-admin-accent{,-light,-dark}` in index.css @theme (static values — identical rendering) and all bg-/text-/from-/to-/focus:ring-/focus:border-/hover:bg- variant sites migrated across admin.tsx + 7 admin components. Exactly ONE arbitrary hex remains app-wide: `hover:bg-[#5558E6]` (CateringPanel.tsx:205, one-off hover shade, documented).
2. **Z-index scale (finding 4.6):** canonical scale documented in index.css (40 chrome / 55 click-away / 56 menus / 50 base modals / 70 stacked / 80 toasts / 200 messages-legacy-ceiling) with the policy: new code uses the scale; existing off-scale values migrate when touched; messages' z-[200] folds down during decomposition.
3. **Inline error states (finding 4.2):** feed/events/housing lists now show a styled inline error block (WifiOff/AlertCircle icon + "Couldn't load X" + Retry button matching each page's empty-state and primary-button patterns) when a load FAILS and the list is empty. Rules held: background-refresh failures with visible content keep showing stale data; feed's subscription error wires through the onSnapshot error callback with a `reloadKey` retry that preserves unsubscribe discipline; housing retry reuses the Session 49 first-page loader (resets cursor correctly). All console.error/toast behavior kept.

**Verified:** tsc exit 0 throughout (both agents + orchestrator runs).
**Ship: web + Android.** Commit `src/`, `firebase deploy --only hosting` (test-gated), `npm run cap:sync` + Run ▶.
**Test:** admin section renders identically (accent red unchanged — pure tokenization); airplane-mode the device → open feed/events/housing → inline "Couldn't load" + Retry appears; reconnect → Retry loads.
...
**Session 56 addendum — search UX fixes from iPad video feedback (pre-deploy):** User's test video (iPad Safari) showed two problems with chat search: (1) screen bounced up/down on every keystroke — root cause: MessageSearchBar's auto-scroll effect fired whenever `matches` recomputed (every letter typed) and scrollIntoView'd the first match. FIXED: navigation is now EXPLICIT only — ▲▼ buttons and Enter ("Go" on mobile keyboards) jump to matches; typing never scrolls. (2) matched TEXT not highlighted — the tranche-4 bubble ring may not have been on the tested bundle, but the user's expectation is text-level highlighting anyway. ADDED: `renderFormattedText(text, highlightQuery?)` third pass wraps case-insensitive matches in plain segments with an amber `<mark>` (dark-mode aware); wired from both bubble text render sites via `isSearchMatch ? searchQuery : undefined`. Limitation (documented): matches inside bold/italic/code/link segments aren't marked — the bubble ring still flags those messages. tsc clean. Ship web + Android; iPad re-test: type → NO scrolling, matched words show amber marks + bubbles ringed; Enter/▲▼ = the only jumps; X clears everything.

**Session 56 addendum 2 — search fixes VERIFIED on Android (July 5):** After the missed-sync was caught (android assets were a day stale — second occurrence of the skipped copy step), user re-synced and confirmed on device: typing no longer scrolls the screen, matched words show the amber <mark> highlight, ▲▼/Enter navigation works. Tranche 4 + search UX fixes fully device-verified; domain 5 (wallpaper/appearance) is next. Stale-bundle self-check documented: `grep -c "bg-amber-300" android/app/src/main/assets/public/assets/messages-*.js` (≥1 = current) — generalize the pattern: grep the newest change's fingerprint in android assets BEFORE device-testing.

---

### Session 57 (July 5, 2026) — D1 Tranche 5: Chat Appearance Domain

NEW `hooks/useChatAppearance.ts` (domain 5): owns wallpaper (localStorage load-on-mount + save-on-select, same 'selectedWallpaper' key + WALLPAPER_PRESETS guard), picker open state, and compact density. The page's mount `useEffect` for wallpaper moved into the hook; WallpaperPicker's inline onSelect (setState + localStorage.setItem) collapsed to `selectWallpaper`. Call sites: `openWallpaperPicker()`, `toggleCompactMode()`, `closeWallpaperPicker`. messages.tsx **57→54 useState** (76 at start; domains 1–5 done). PARITY NOTE: compactMode kept NON-persisted (resets each mount, exactly as before) — persisting it is an obvious future enhancement but is a behavior change, deliberately left out. Plan doc updated; next = domain 6 (forward/lightbox, 6 states, Med risk — first reducer-candidate tranche).

**Verified:** tsc exit 0. Ship web + Android (`npm run cap:sync` — remember the copy step!).
...
**Session 63 addendum — bubble tail removed (user-reported cosmetic):** the WhatsApp-style CSS-triangle "tail" on the first message of each group (messages.tsx ~3422) read as a stray arrow at the top-left of the pill-shaped call/voice bubbles. Per user choice (remove → clean rounded), deleted the tail `<div>` and simplified the bubble corner logic from the split `rounded-tl-lg rounded-tr-sm/rounded-tr-lg rounded-tl-sm + rounded-b-lg` (with a flat tail-side corner) to a uniform `rounded-2xl` on all bubbles. Pre-existing element, NOT from any tranche. tsc clean. If 16px (rounded-2xl) feels too round after seeing it live, dial to rounded-xl (12px) or rounded-lg (8px) — one word. Ships with the E2EE log fix via `npm run ship`.

---

### Session 64 (July 5, 2026) — D1 Tranche 11: E2EE + Calls State — THE FINALE 🏁

NEW `hooks/useCallAndE2EEState.ts` (domain 11, the last one): the 5 highest-risk states — e2eReady, e2eKeyVersion, callState, groupCallState, activeGroupCallId. STATE-only extraction, raw setters/identical names → all call sites byte-unchanged. callState/groupCallState seed from `getCallManager()/getGroupCallManager().getState()` (exact parity with prior init — a call already in progress shows immediately on mount). EVERYTHING that touches the singletons stays in the page: callManagerRef/groupCallManagerRef (verified present), both `.subscribe()` effects (verified present), the active-group-call onSnapshot, the ECDH key-init + shared/group-key derivation effects, all call/join handlers.

**🎉 messages.tsx STATE DECOMPOSITION COMPLETE: 76 → 1 useState.** The lone survivor is `showChatMenu` (a trivial 3-dots menu toggle — deliberately left, not worth a hook). All 11 domains now live in 11 focused hooks (useChatNotification, useChatModeration, useMessageActions, useChatSearch, useChatAppearance, useForwarding, usePinnedAndDisappearing, useGroupManagement, useComposer, useChatData, useCallAndE2EEState). The 12-tranche program (Sessions 50–64) also surfaced & fixed several latent bugs along the way (dead chat-search highlight, ETA validation, top-level replies, E2EE log noise).

**Verified:** tsc exit 0; 0 domain-11 states left as raw useState; both singleton refs + both subscribe effects intact.
**⚠️ HIGHEST-STAKES DEVICE TEST — calls + E2EE are the most expensive to break:** (1) 1:1 voice call — place, ring, answer, talk, hang up; (2) 1:1 video call; (3) group call — start, join from the "call in progress" banner, leave; (4) call header buttons disable correctly while a call is active (callState.status !== idle); (5) E2EE: send/receive in a 1:1 (messages decrypt), send/receive in a group, and confirm a just-derived key triggers re-decryption (e2eKeyVersion bump); (6) console clean (post-Session-63 the decrypt noise is gone — any error now is REAL). Run `npm test` on Mac too (encryption.test.ts crypto utils untouched → 38 should pass).
**Ship web + Android:** `npm run ship` then Run ▶.

**REMAINING (not state — architecture):** companion 10b — extract the 6 onSnapshot subscriptions + msgSnapshotSeqRef guard + E2EE key-derivation effects into `services/messages.ts`. Separate focused session; the state layer is now fully done, so 10b is a cleaner target than before.

*Updated July 5, 2026 (Session 64) — 🏁 messages.tsx state decomposition COMPLETE: tranche 11 (useCallAndE2EEState) done, 76→1 useState across 11 hooks. Only companion 10b (services/messages.ts I/O extraction) remains, and it's architectural not state. tsc clean; NOT committed; ship via npm run ship + the highest-stakes call/E2EE device test.*

---

### Session 65 (July 5, 2026) — Two user-reported issues: group call (infra, diagnosed) + group avatar (feature, built)

**Issue 1 — Group call audio/video "doesn't work": NOT a tranche-11 regression; it's the known free-TURN gap.** Confirmed GroupCallOverlay is rendered in MainLayout.tsx (`<GroupCallOverlay />`, no props) and subscribes to the GroupCallManager singleton DIRECTLY — fully independent of the groupCallState that tranche 11 moved (that copy only drives header-button disable). Root cause: `utils/iceConfig.ts` runs on the FREE public `openrelay.metered.ca` TURN unless `VITE_TURN_URLS/USERNAME/CREDENTIAL` are set in `.env`. That free relay is unreliable and WORST for group calls (mesh = N-1 peer connections per participant, multiplying TURN load) — matches "audio+video don't flow." This is the Session-43 carry-forward ("replace free TURN with paid provider"). **FIX = infrastructure, not code**: get a TURN provider (metered.ca paid / Twilio NTS / Cloudflare / self-hosted coturn), set the 3 VITE_TURN_* env vars, rebuild. The code already reads them (iceConfig.ts:31-43); getIceServers() is pre-wired for the future ephemeral-credential Cloud Function upgrade. NOT fixed here (needs user's provider account). Diagnostic to confirm: does 1:1 call work? If 1:1 works but group fails → TURN-under-mesh-load (same fix). If both fail → same TURN fix, more urgent.

**Issue 2 — Group avatar had no edit option: FEATURE BUILT.** Groups only ever showed a hardcoded Users icon; Conversation type had no avatar field. Added (agent-built, orchestrator-verified): `groupAvatar?: string` on Conversation type; `updateGroupAvatar(value)` handler (admin-guarded, updateDoc + system message + toast, mirrors updateGroupName); `groupAvatarInputRef` + handler (10MB guard, compressImage(file,512,0.75)→base64, quota-safe); admin-only edit UI in Group Info (Change Photo + 16-emoji AVATAR_OPTIONS grid + Remove); avatar-or-icon display (img if http/data, emoji if set, else Users fallback) at 3 sites — Group Info circle, room header, conversation-list tile. Non-admins see the avatar read-only. tsc clean; messages.tsx still 1 useState (decomposition intact).

**Ship web + Android** (`npm run ship`). **Test:** admin sets group photo/emoji → shows in Group Info + header + list + system message; Remove reverts to icon; non-admin sees read-only; >10MB rejected. Group call: separate — needs the TURN env vars.

*Updated July 5, 2026 (Session 65) — group-call "broken" diagnosed as the free-TURN infra gap (NOT tranche 11; GroupCallOverlay is singleton-driven, independent of moved state) — fix is setting VITE_TURN_* to a real provider. Group-avatar edit feature BUILT (field + admin edit UI photo/emoji/remove + display at 3 sites). tsc clean; NOT committed; ship via npm run ship.*

**Session 65 addendum — TURN setup path prepared (group-call fix):** confirmed group mesh signaling code is STRUCTURALLY CORRECT (offer/answer/ICE bidirectional both sides, tracks added, ontrack handlers, candidate serialization matches) — NOT a code bug and NOT tranche 11. 1:1 works but group doesn't → free openrelay TURN buckling under mesh's N-1 relay allocations. Wrote `docs/TURN_SETUP.md`: step-by-step for Metered.ca free tier → fill VITE_TURN_URLS/USERNAME/CREDENTIAL in .env (already present, empty; gitignored) → rebuild (env baked at BUILD time, MUST rebuild) → deploy. Recommends `turns:` (TLS) URL first for encrypted relay channel + firewall traversal. Security confirmed to user: WebRTC media is always DTLS-SRTP E2E encrypted; TURN relays ciphertext only, never decrypts; mesh = per-pair encryption (more private than SFU); messages E2EE is a separate untouched layer. Code (`iceConfig.ts`) already reads the env vars + has getIceServers() stubbed for future ephemeral-credential Cloud Function hardening. USER ACTION: sign up Metered → paste creds into .env on Mac → npm run ship. If group still fails after real TURN, capture [GroupCall] Connection-state console logs (failed=still ICE, connected-but-no-media=overlay bug).

---

### Session 66 (July 5, 2026) — Group call REAL root cause found + FIXED: no incoming ring for group calls

**Metered TURN was already deployed and WORKING — ruled it out via live Chrome diagnostics.** Connected to the deployed enovoapp.com in Chrome, started a group audio call, captured `[GroupCall]` console + ran a live ICE test:
- `getUserMedia` OK, room created, ZERO JS errors on the start path.
- **Live TURN reachability test** (throwaway RTCPeerConnection with the Metered creds against the deployed site) gathered **4 `relay` candidates** from Metered's relay IP (64.227.188.203). TURN is reachable + creds valid.
- Live bundle (`groupWebrtc-*.js`) confirmed to contain `global.relay.metered.ca` + the username → deploy DID pick up the new `.env`.
- Firestore rules for `groupCalls/**` signaling + candidates are fully permissive.
- Mesh offer/answer/ICE code in groupWebrtc.ts is structurally correct for a 2-party join.

**ACTUAL ROOT CAUSE (why 1:1 works but group doesn't):** group calls had **no incoming-call ring**. 1:1 has `GlobalCallOverlay` → `callManager.listenForIncomingCalls` (mounted app-wide) that rings the callee ANYWHERE + a Cloud Function `sendIncomingCallNotification` (calls/{callId}) push. Group calls had NO equivalent — the only join path was the in-chat "Group call in progress — Tap to join" banner (messages.tsx:300 effect), which requires the other member to already have that exact group chat open. So when you start a group call, other members are NEVER alerted → nobody joins → no peer connects → "audio+video don't work." TURN/media/rules/signaling were all fine; the second peer just never entered the room.

**FIX BUILT (5 files, mirrors the 1:1 incoming pattern):**
1. `utils/groupWebrtc.ts` — added `'ringing'` status + incoming metadata (`incomingRoomId/FromName/CallType/ConversationId`) to GroupCallState; ringtone (copied from CallManager); `listenForIncomingGroupCalls(myUid, myName, onIncoming)` = onSnapshot on `groupCalls` where `memberUids array-contains myUid && status==active` (skips own call/busy/already-handled/already-participant/stale >45s, auto-dismiss after RING_TIMEOUT_MS, dismisses if room ends); `acceptIncoming()` (→ joinCall) + `declineIncoming()` (local dismiss); `handledIncomingRooms` Set so a room never re-rings. `startCall` now takes `memberUids: string[]` and writes `memberUids` + `createdByName` on the room doc (self-created/joined rooms added to handledIncomingRooms).
2. `components/GroupCallOverlay.tsx` — mounts `listenForIncomingGroupCalls` on user; renders a full-screen ring card (caller name/initials, "Incoming group audio/video call", green Accept + red Decline) when `status==='ringing'`.
3. `pages/messages.tsx` — `initiateGroupCall` passes `activeGroupConv.participants` as memberUids into startCall.
4. `functions/src/index.ts` — NEW `sendIncomingGroupCallNotification` onDocumentCreated(`groupCalls/{roomId}`): HIGH-priority FCM to all memberUids EXCEPT creator (gathers each member's fcmTokens, dedups, prunes stale), reuses `type: "incoming_call"` so push.ts routing already handles the tap. Background/closed-app ring.
5. `firestore.indexes.json` — added composite index `groupCalls` (memberUids CONTAINS + status ASC) for the listener query.

**Verified:** app `tsc --noEmit` exit 0; functions `tsc --noEmit` exit 0; full vitest suite **270/270 pass**. NOT committed. Build/deploy is Mac-only (Linux sandbox lacks the rollup native binary).

**USER ACTION — deploy all three targets:** (a) `npm run ship` (web + Android bundle with the new ring UI), (b) `firebase deploy --only functions` (the new group-call push), (c) `firebase deploy --only firestore:indexes` (the new composite index — the ring listener query needs it, else it errors until built). **Test with your 2 devices:** start a group call on device A → device B should RING (full-screen Accept/Decline) even if it's on another screen → Accept → both should now see/hear each other (TURN already works). Also test Decline, and the in-chat banner still works as a fallback.

*Updated July 5, 2026 (Session 66) — group call REAL root cause: NO incoming ring for group calls (1:1 had it, group didn't) — TURN/media/rules/signaling all verified fine via live Chrome diagnostics (4 relay candidates gathered). FIXED across 5 files: listenForIncomingGroupCalls + ring UI + accept/decline + memberUids on room + Cloud Function push + Firestore index. tsc clean both projects, 270/270 tests pass; NOT committed. Deploy needs ship + functions + firestore:indexes.*

---

### Session 66 addendum — deploy execution, gotchas, and where we stopped

This addendum makes Session 66 a complete, continuous record (the entry above was written pre-deploy). Read this for the CURRENT state.

**(1) What we were building/working on this session.** One focused goal: make **group audio/video calls actually work**. Prior sessions had (correctly) wired a real Metered.ca TURN server but group calls still failed. This session diagnosed the true cause and built the fix, then attempted deploy.

**(2) Key decisions + WHY.**
- *Diagnose with live Chrome before touching code.* Connected to deployed enovoapp.com, started a group call, read `[GroupCall]` console, and ran a throwaway RTCPeerConnection ICE test with the Metered creds. Rationale: don't guess — prove which layer is broken. Result: TURN gathered **4 relay candidates** (works), media/room/rules/signaling all fine → ruled every network/config cause OUT.
- *Root cause = no incoming ring for group calls.* 1:1 has `GlobalCallOverlay → listenForIncomingCalls` (rings anywhere) + a Cloud Function push. Group had neither — the only join path was the in-chat "call in progress" banner, which needs the other member to already be looking at that group. So callers sat alone. Decision: mirror the proven 1:1 pattern rather than invent a new mechanism.
- *Ring the whole group via `memberUids array-contains`.* A group call has no single callee, so the room doc now carries `memberUids` (all members) and `createdByName`; the listener queries active rooms containing me. Chosen over client-side cross-referencing of conversations because it's a single indexed Firestore query and mirrors 1:1's `calleeId` filter.
- *Reuse `type: "incoming_call"` in the push payload* so existing `native/push.ts` routing (→ /messages) already handles the tap; the actual ring is driven by the Firestore listener, so no new push-handling code was needed.
- *`turns:` (TLS) stays first + DTLS-SRTP unchanged* — the user's hard constraint that all connections stay encrypted is preserved; TURN only relays ciphertext.

**(3) Completed this session.** The full fix, code-complete + verified: 5 files changed — `utils/groupWebrtc.ts` (`'ringing'` status + incoming metadata on GroupCallState, ringtone copied from CallManager, `listenForIncomingGroupCalls`/`acceptIncoming`/`declineIncoming`, `handledIncomingRooms` guard, `startCall(...memberUids)` writing `memberUids`+`createdByName`), `components/GroupCallOverlay.tsx` (mounts the listener + full-screen ring card), `pages/messages.tsx` (`initiateGroupCall` passes `activeGroupConv.participants`), `functions/src/index.ts` (`sendIncomingGroupCallNotification` onDocumentCreated groupCalls/{roomId}), `firestore.indexes.json` (groupCalls memberUids CONTAINS + status composite index). App tsc 0, functions tsc 0, **270/270 tests pass**. **Committed** as `f7e3f15` ("Group calls: incoming ring…") — 7 files, +532/−2 (includes docs/TURN_SETUP.md from Session 65).

**(4) In progress / half-done — DEPLOY IS NOT FINISHED.** The commit is local only and the app is NOT yet deployed. Two blockers hit during deploy:
  - **git push failed — GitHub auth.** `remote: Invalid username or token. Password authentication is not supported`. The commit is safe locally; push needs a Personal Access Token or `gh auth login` (GitHub disabled HTTPS password auth). Deploy does NOT depend on the push (Firebase/Capacitor build from local files).
  - **`firebase deploy --only hosting` failed — rollup native binary.** Error: `Cannot find module @rollup/rollup-darwin-arm64`. **Cause (own it): I ran `npm install @rollup/rollup-linux-arm64-gnu --no-save` in the Linux sandbox to run the test suite, and because the sandbox shares the SAME mounted `node_modules` as the Mac, it displaced the macOS rollup binary** (verified: `node_modules/@rollup/` now has `rollup-linux-arm64-gnu`, not `rollup-darwin-arm64`; but `package-lock.json` is CLEAN and still lists darwin-arm64 in 3 places). **⚠️ LESSON for future sessions: do NOT `npm install` into the sandbox — it mutates the user's real node_modules.** Run only read/verify/tsc/tests that don't reinstall platform-specific deps, or accept that tests may not run in-sandbox.

**(5) EXACT next steps (run on the Mac).** One chained command — clean-reinstall from the intact lockfile, then the full deploy:
```bash
cd /Users/sarathsatheesan/ethniCity_03_19_2026/sangam-pwa-v2 && \
...
echo "✅ Deployed"
```
`npm ci` restores `@rollup/rollup-darwin-arm64` exactly per package-lock (no version drift). If npm ci still trips the npm optional-deps bug, fallback: `rm -rf node_modules package-lock.json && npm install`. Then Android Studio → **Run ▶**. Fix git push separately: `gh auth login && git push` (or create a PAT and `git push` using the token as the password). **After the `firestore:indexes` deploy, wait for the index to reach "Enabled" in Firebase console → Firestore → Indexes before testing** — the ring listener query errors until it's built.
  **Device test:** start a group call on device A → device B should ring full-screen (Accept/Decline) even on another screen → Accept → audio/video should flow (TURN already confirmed working). Also test Decline; the in-chat "call in progress" banner remains as a fallback join path.

**(6) Important context / constraints / files.**
  - *Deploy targets are THREE, not one:* `npm run ship` only does hosting+Android (its script = `npm test && npm run build && npx cap sync android && firebase deploy --only hosting`). The new **Cloud Function** and **Firestore index** need explicit `firebase deploy --only functions,firestore:indexes`. **Functions have NO predeploy build hook in firebase.json**, so `functions/` must be compiled (`npm --prefix functions run build`) before deploying or the new function won't ship. Hosting DOES have a predeploy (`npm test && npm run build`) so it self-builds + gates on the 270 tests.
  - *Env/secrets:* `.env` (gitignored) holds the working Metered TURN creds (`VITE_TURN_URLS/USERNAME/CREDENTIAL`) + Firebase VAPID + Google Maps key. Vite bakes `import.meta.env.*` at BUILD time → any `.env` change requires a rebuild. Confirmed the LIVE bundle already contains `global.relay.metered.ca` + the username.
  - *Encryption constraint (user's non-negotiable):* WebRTC media is DTLS-SRTP E2E encrypted; TURN relays ciphertext only; group mesh = per-pair encryption (more private than an SFU); `turns:` TLS for the relay channel. Messages E2EE is a separate untouched layer.
  - *Design choices introduced this session (new):* the incoming group-call ring screen — full-screen dark gradient (`#0f0f1a → #1a1b2e → #2d2f5e`), pulsing purple-gradient avatar circle (`#7e22ce → #4f46e5`) with initials, green **Accept** (`#22C55E`, Phone or Video icon by call type) + red **Decline** (`#EF4444`, PhoneOff), green "Encrypted" shield header. Matches the existing GroupCallOverlay visual language.
  - *Unchanged this session (still authoritative — see Section 1 of this note):* **Tech stack** React 19.2 + TS strict, Vite 7.3 + PWA, Tailwind v4, Firebase (Firestore/Auth/Storage/Hosting/Functions/FCM), Capacitor Android, WebRTC, Web Crypto E2EE. **Design system** "Aurora" (CSS vars `--aurora-*`/`--msg-*`), primary **Delta Navy #0032A0** + **Delta Red #C8102E** on **#F5F7FA**; dark mode via `.dark` on `<html>`. Messages header purple gradient `#7e22ce → #7c3aed → #4f46e5`. **Tone:** concise/direct, minimal formatting. **Build/deploy is Mac-only** (rollup native binary); tests gate deploy (270).
  - *Still open / carry-forward (not blocking):* companion **10b** — extract the 6 onSnapshot subscriptions + `msgSnapshotSeqRef` + E2EE key-derivation effects into `services/messages.ts` (architectural, messages.tsx state is already 76→1). Business carry-forwards: shareable APK, Play Store submission. Optional TURN hardening: ephemeral per-call credentials via a Cloud Function (`iceConfig.ts getIceServers()` already stubbed) — defer until usage grows; set a Metered budget alert meanwhile.

*Updated July 5, 2026 (Session 66 addendum) — group-call incoming-ring fix committed (f7e3f15) but NOT yet deployed: git push blocked on GitHub token auth, and hosting build blocked on a missing macOS rollup binary that MY sandbox `npm install` displaced (package-lock is clean → `rm -rf node_modules && npm ci` fixes it). Exact resume command + 3-target deploy (hosting / functions / firestore:indexes) documented above. Lesson recorded: never npm-install into the shared sandbox node_modules.*

---

### Session 67 (July 5, 2026) — GitHub Push Restored + Deploys Verified + Group-Call Ring/Video FIXED (device-confirmed)

**(1) GitHub push unblocked.** Root cause of the auth failure: the PAT embedded in the remote URL was the **expired "Ethnicity" token** (reads still worked because the repo is public — that masked it). Created fine-grained PAT **`sangam-push`** via Chrome (repo-scoped to sangam-pwa-v2, Contents: Read and write, expires **Aug 4 2026** — renew then). Gotchas hit: token pasted 4× into the remote URL (hidden `read -s` input; fixed by dedup), and the **sandbox cannot delete files in `.git`** (create OK, unlink EPERM) — stale `index.lock`/`main.lock` accrue from sandbox git commands and must be `rm`'d on the Mac; commits happen on the Mac, sandbox can push/read. All 8 commits pushed; `origin/main` = `ee67b31` then the Session-67 fix commit.

**(2) All three Session-66 deploy targets verified LIVE via Firebase console:** hosting (live site serves dist's exact entry chunk), functions (`sendIncomingGroupCallNotification` us-west3, fired on the first test call), firestore index (`groupCalls` memberUids CONTAINS + status — Enabled).

**(3) Group-call ring "still broken" → diagnosed from room-doc forensics, NOT code.** Tablet-created rooms had NO `memberUids`/`createdByName`; Mac-created rooms had both → **the tablet was running the pre-S66 install** (assets synced 22:07 but Run ▶ never done — 4th stale-Android incident). One stale device kills ring BOTH directions: as caller it writes rooms receivers can't match; as callee it has no listener. Fix = Run ▶. **Diagnostic to remember: check the newest `groupCalls` doc for `memberUids` — instantly identifies which SIDE is stale.**

**(4) Remote-video autoplay bug FIXED (`GroupCallOverlay.tsx` ParticipantTile, only file changed):** remote tiles attached the FULL stream (audio included) unmuted → Chrome/Android blocked autoplay (native play-button overlay over the feed, seen in user's photo) and remote audio double-played (video element + dedicated audio element). Now: remote video element gets a **video-only `new MediaStream(stream.getVideoTracks())` and is always `muted`** (audio path is solely the hidden `<audio>` element); effect also re-runs on `videoTrackCount` so a video track arriving late on the same stream object still attaches (previously could leave a black tile — likely the "tablet video not visible on receiver" variant). tsc exit 0.

**(5) OUTCOME: user confirmed on devices — ring works both directions, remote video renders, issues fixed.** Shipped via `npm run ship` + Run ▶.

**Carry-forwards unchanged:** companion 10b (services/messages.ts I/O extraction), shareable APK / Play Store, TURN ephemeral-credential hardening, `:any` burn-down, opportunistic re-verify of the S52 report-dedupe check.

*Updated July 5, 2026 (Session 67) — push auth fixed (new PAT, old one had expired), S66 deploys verified live, ring failure root-caused to a stale tablet install via room-doc forensics, remote-video autoplay/double-audio/late-track bugs fixed in ParticipantTile. Device-confirmed working. Note: hand_off_note edits from this session need a Mac-side commit.*

---

### Session 67b (July 5, 2026) — Companion 10b: services/messages.ts — THE MESSAGES DECOMPOSITION IS FULLY DONE 🏁

NEW `services/messages.ts` (522 lines, feed.ts conventions header: mechanical moves, page keeps try/catch + toasts, subscribeX returns unsubscribe, crypto primitives stay in utils/encryption). Everything Firestore in messages.tsx moved:
1. **`subscribeToThreadMessages(opts)`** — the render backbone: snapshot handler + the FULL 4-strategy decrypt pipeline (deterministic → cached ECDH → per-message ECDH [does a getDoc mid-stream, hence service-side] → legacy v1) + the mark-visible-unread batch. The **msgSnapshotSeqRef race guard is passed IN as the page's long-lived `{current}` ref** — it must survive re-subscribes so a stale async handler sees bumps from its replacement. Page keeps: same effect deps (incl. the exhaustive-deps disable), setMessagesLoading(true), and applies state + the initial-load pin-to-bottom retry scroll via `onMessages(msgs, initialLoad)`. Group-ness is computed at effect-creation from the closed-over conversations — SAME stale-closure semantics as before (handler previously read the effect's fixed closure).
2. `subscribeToConversations`, `subscribeToActiveGroupCall` (banner listener; GroupCallManager.getActiveCall initial check stays in page), `subscribeToPresenceOfUsers` (batched 'in'-queries of 10, one combined unsub; page keeps the setUsers merge), `startPresenceHeartbeat` (entire writer lifecycle: online/heartbeat/away/offline + visibilitychange/beforeunload/pagehide), `fetchAllUsers`, `markThreadRead` (visibilitychange handler body; listener + visibility check stay in page).
3. **E2EE effects — Firestore half only** (parity-preserving split): `publishPublicKey`, `fetchPeerPublicKey`, `fetchGroupKeyMaterial` (wrapped key + distributor pubkey). getOrCreateKeyPair/deriveSharedKey/unwrapGroupKeyWithECDH calls and the cancelled-flag placements stay in the page EXACTLY where they were — no behavior change to key init/derivation ordering.

**Result:** messages.tsx 4367→4079 lines, **0 onSnapshot / 0 writeBatch / 0 presence constants in the page**; unused imports pruned (onSnapshot, writeBatch, decryptMessage, e2eDecrypt, PRESENCE_*, PresenceStatus). tsc exit 0; eslint clean on the new service (page's pre-existing lint debt untouched). Plan doc updated — 10a+10b both ✅, program CLOSED.

**NOT committed; NOT device-tested. Ship web + Android (`npm run ship` + Run ▶) and run the 10a full-regression checklist again (it exercises exactly this code):** conversation list loads + live-updates; open a 1:1 AND a group; send/receive (E2EE decrypts — console should stay quiet per S63); presence dots/last-seen update; unread counts clear on open + on tab-return; pinned banner reflects pins; group-call banner appears for an active call; disappearing/starred/search still fine. `npm test` on Mac must stay 270/270 (crypto utils untouched).
...
**SECURITY REMEDIATION (Session 68, July 6 2026) — see `../eNoVo_Security_Review_07_05_2026.md` §7 for the full ledger.** A static security review found 4 Critical / 6 High / 7 Medium / 3 Low. Fixes applied in risk order (each collection's legit write paths verified in code before locking):
- **SHIPPED (rules + config, NOT deployed): C3** (appConfig write→admin, closes admin self-escalation), **C4** (messages subcollection participant-scoped), **M7** (conversation delete scoped), **H2-partial** (verified/verificationMethod admin-only + business delete owner/admin), **H3** (banned/disabled admin-only), **H4-read** (notifications recipient-scoped), **M3** (announcements/adminSettings admin-only; moderationQueue deliberately left open), **M2** (security headers in firebase.json), **L1** (OTP phone-log masked). Files: `firestore.rules`, `firebase.json`, `services/auth.ts`. tsc 0, JSON valid, braces balanced. **Deploy: `firebase deploy --only firestore:rules` + `--only hosting` — but VALIDATE ON STAGING with the report §4.7 exploit checklist FIRST (rules can't be compiled offline).**
- **STAGED (do NOT ship blind — need E2EE test suite + device test + migration decisions):** **C1+C2+H5+M1** (the E2EE redesign — the current "E2EE" is NOT end-to-end: private keys are stored server-side world-readable AND the default message key is `PBKDF2(public UIDs + bundle salt)`, recomputable by anyone; M1 = crypto fails open to plaintext mislabeled `encrypted:true`). **H1** (KYC TIN/beneficial-owners are plaintext on world-readable business docs — relocate + migrate). **H4-create** (notification-injection callable). **M4** (transcribe membership guard). **H6** (ephemeral TURN).
- **CONFIG/OPS (owner action):** M5 (restrict Maps API key), CSP build+rollout, L2 (guard verbose logs), L3 (App Check).

> ⚠️ Until C1+C2 are fixed, do not advertise messaging as "end-to-end encrypted" — it is transport-encrypted only. This is the top security priority above all feature work.

---

### Session 68 (July 5–6, 2026) — Orientation + Group-Call Ring/Video Fix + Companion 10b + Full Security Review & Remediation

This was a long multi-part session. It is written as a continuous record so it can be resumed cold.

**(1) What we were working on.** Four distinct pieces, in order: (a) fresh-session **orientation** to the project from this handoff note; (b) closing out **Session 66's group-call deploy tail** (GitHub push + verifying the three Firebase deploys) and fixing a **user-reported group-call bug** (no ring + remote video invisible on Mac); (c) **companion 10b** — the last architectural piece of the messages decomposition; (d) a **comprehensive security review** of the whole app (client-requested: acting as a security consultant/pentester/DevSecOps) followed by **risk-ordered remediation** of the Firestore rules and config.

**(2) Key decisions and WHY.**
- *GitHub auth root cause = expired token, not a config error.* The push had been failing because the PAT embedded in the remote URL was the **expired "Ethnicity" token**; reads worked only because the repo is public, which masked it. Created a new fine-grained PAT `sangam-push` (Contents:RW, repo-scoped, **expires Aug 4 2026**) via Chrome. Decided to keep commits Mac-side (the sandbox can create but not delete files under `.git`, so it accrues stale `index.lock`/`main.lock` that must be `rm`'d on the Mac; the sandbox CAN push/read).
- *Group-call ring failure diagnosed from Firestore room-doc forensics, NOT guessed.* Tablet-created `groupCalls` rooms lacked `memberUids`/`createdByName` while Mac-created ones had them → proved the **tablet was on a pre-Session-66 install** (assets synced but Run ▶ never done — the 4th stale-Android incident). A single stale device breaks ringing in BOTH directions. Decision recorded: *check the newest `groupCalls` doc for `memberUids` to instantly identify which SIDE is stale.*
- *Remote-video overlay bug (Mac) was a real code bug, fixed.* The photo showed Android's autoplay "play button" over the remote tile. Root cause: remote `<video>` got the FULL stream (incl. audio) **unmuted** → Chrome/Android block autoplay + double-play audio. Fix: remote tiles now attach a **video-only, always-muted** MediaStream (audio plays solely via the dedicated `<audio>` element), and the attach effect re-runs on `videoTrackCount` so a late-arriving video track still binds. One file: `components/GroupCallOverlay.tsx`.
- *10b done as mechanical moves only, with the race guard passed in by ref.* Followed the `services/feed.ts` conventions. The `msgSnapshotSeqRef` monotonic guard is passed INTO `subscribeToThreadMessages` as the page's long-lived `{current}` ref so a stale async handler still sees bumps from its replacement. Group-ness computed at effect-creation preserves the prior stale-closure semantics exactly.
- *Security remediation ordered by RISK, and every rule verified against real client-write paths in code BEFORE locking.* This caught traps that a naive lock would have broken: `moderationQueue` (normal users file reports → left open), `businesses` update (reviews/reports write it → only the `verified`/`verificationMethod` trust-fields locked to admin, not the whole doc), `notifications` create (the whole app enqueues them → left open with a tracked-risk note; only READ tightened). `appConfig` locked to `isAdminUser()` rather than `false` so the admin panel keeps working while closing the escalation.
- *The E2EE redesign was deliberately STAGED, not shipped.* The two Critical crypto findings (C1 server-stored world-readable private keys; C2 default message key = `PBKDF2(public UIDs + bundle salt)`, recomputable by anyone) are coupled and touch the repo's highest-risk file with no live-test capability here. Decided to write the exact fix design into the report rather than half-ship a crypto migration that could strand all message history.

**(3) Completed this session.**
- **GitHub:** all 8 backlog commits + the aftermath commit pushed; `origin/main` current. New PAT created and set on the remote.
- **Firebase deploys VERIFIED live** (via console): hosting (live entry chunk matches dist), the `sendIncomingGroupCallNotification` function, and the `groupCalls` composite index (Enabled).
- **Group-call fix** (ring + remote video) — code-complete, tsc 0, **user-confirmed working on devices**.
- **Companion 10b** — `services/messages.ts` (522 lines) extracts ALL messages Firestore I/O; `messages.tsx` 4367→4079, zero inline `onSnapshot`. tsc 0. Plan doc marked CLOSED (the 12-tranche program is fully done: 76→1 useState + all I/O extracted).
- **Security review** — `../eNoVo_Security_Review_07_05_2026.md`: 4 Critical / 6 High / 7 Medium / 3 Low, each with file:line evidence + a pentest checklist (§4.6 MitM, §4.7 authenticated-exploit).
- **Remediation SHIPPED to the tree (NOT deployed):** `firestore.rules` (C3 appConfig admin-only, C4 messages participant-scoped, M7 conv-delete scoped, H2 verified-field + business-delete locked, H3 banned/disabled admin-only, H4 notifications recipient-scoped read, M3 announcements/adminSettings admin-only), `firebase.json` (M2 security headers), `services/auth.ts` (L1 OTP phone-log masked). tsc 0; firebase.json valid; rules braces balanced.

**(4) In progress / half-done.**
- **Nothing is deployed.** All Session-67/67b/68 code + rules + the two report .md files are **uncommitted** and unshipped. The messages 10b refactor is **not yet device-tested**.
- **STAGED (specced, not coded):** the E2EE redesign (C1+C2+M1+H5), KYC PII relocation (H1), notification-injection callable (H4-create), transcribe guard (M4), ephemeral TURN (H6). Full spec in report §7.

**(5) EXACT next steps (Mac).**
1. **Clear sandbox locks + commit everything:**
   ```bash
   cd /Users/sarathsatheesan/ethniCity_03_19_2026/sangam-pwa-v2
   rm -f .git/index.lock .git/refs/remotes/origin/main.lock fix_push.sh
   git add src/ services/ docs/ firestore.rules firebase.json hand_off_note_04_16.md
   git commit -m "session 68: 10b messages I/O extraction + security rules/headers/log hardening"
   git push origin main
   ```
2. **Ship the 10b refactor (code):** `npm run ship` + Android Studio Run ▶ → run the messages full-regression checklist (list live-updates, 1:1 + group send/receive with quiet console, presence dots, unread clears, pinned/group-call banners). Revert pair if the backbone misbehaves: `git checkout src/pages/messages.tsx src/services/messages.ts`.
3. **Deploy the security rules — VALIDATE FIRST (rules can't be compiled offline; no emulator set up):**
   - Firebase Console → Firestore → Rules → **Playground**: simulate a non-participant `get` on `conversations/{id}/messages/{m}` → must DENY; a non-admin write to `appConfig/settings` → must DENY.
   - `firebase deploy --only firestore:rules` (instant, reversible via console version history).
   - **Smoke-test live:** send/receive in a chat, **start a brand-new conversation and send the FIRST message** (highest-risk — the messages rule does a `get()` on the parent conv doc, which must exist first), post a review, file a report, admin-verify a business.
   - Then `firebase deploy --only hosting` for the M2 headers (test-gated build).
4. **Then the security backlog, recommended order: H1 (KYC relocation — contained, ~1 day) → C1+C2 (E2EE redesign — dedicated session, E2EE test suite as guard, needs a message-history migration decision).** Rationale: once the rules tighten, the recomputable-key crypto weakness becomes the weakest remaining link and the "E2EE" claim is currently false; H1 first is a clean regulatory-grade win before the big redesign. **Meanwhile: pull "end-to-end encrypted" wording from the UI until C1+C2 land.**

**(6) Important context / constraints / files.**
- *New this session:* `services/messages.ts`, `../eNoVo_Security_Review_07_05_2026.md` (the security report + §7 implementation ledger). Modified: `firestore.rules`, `firebase.json`, `services/auth.ts`, `components/GroupCallOverlay.tsx`, `pages/messages.tsx`, `docs/messages-state-decomposition-plan.md`.
- *Sandbox constraint (recurring):* the Linux sandbox can create but NOT delete files under `.git` → stale `.git/index.lock` + `.git/refs/remotes/origin/main.lock` accumulate; `rm -f` them on the Mac or the next commit/push fails. Also **never `npm install` in the sandbox** — it mutates the Mac's shared `node_modules` (Session 66 rollup incident).
- *Deploy targets remain THREE and Mac-only:* `npm run ship` = hosting + Android only; functions + firestore:indexes need explicit `firebase deploy --only functions,firestore:indexes`; **rules deploy separately** via `--only firestore:rules` (no build, instant, reversible). Hosting predeploy runs the 270 tests. Rules CANNOT be validated offline — use the Console Playground.
- *Security posture summary (for whoever picks this up):* TLS baseline is strong; AES-256-GCM with random IV is the right primitive; `.env` is gitignored/untracked; `npm audit` prod = 0. BUT the "E2EE" is not end-to-end (C1/C2), KYC TINs are plaintext on world-readable docs (H1), and rules followed an "authenticated = trusted" model now partially tightened. See report §0 severity table.

**Unchanged this session (still authoritative — repeated so this note stands alone):**
- **Tech stack:** React 19.2 + TypeScript (strict), Vite 7.3 + PWA (vite-plugin-pwa), Tailwind CSS v4 (`@custom-variant dark` with zero-specificity `:where()`), Firebase (Firestore / Auth / Storage / Hosting / Functions 2nd-gen / FCM), Capacitor Android, Framer Motion, Lucide icons, WebRTC (DTLS-SRTP + Metered TURN), Web Crypto + CryptoJS for messaging crypto. Deployed to Firebase Hosting site `mithr-1e5f4`; live at enovoapp.com / ethnizity.com / enovoapp.online/.org/.store + mithr-1e5f4.web.app.
- **Design system — "Aurora"** via CSS variables (`var(--aurora-*)`, `var(--msg-*)`). NOTE: an earlier header claimed primary = Delta Navy `#0032A0` / Delta Red `#C8102E`, but `src/constants/theme.ts` + `index.css` are the reality: **"Modern Aurora"** — primary Electric Indigo `#6366F1` (light `#818CF8`, dark `#4F46E5`), secondary Soft Mint `#10B981`, tertiary Aurora Cyan `#22D3EE`, gradient Indigo→Mint. Backgrounds: light `#F5F6FA` page / `#FFFFFF` cards / `#EDF0F7` inputs; dark surface `#232438` / bg `#1A1B2E`. Text `#1E2132`/`#5B5E72`/`#9295A5`. Semantic: success `#10B981`, danger `#EF4444`, warning `#F59E0B`. Aurora tokens in index.css `@theme`: `--aurora-accent #6366F1`, `--aurora-success`, `--aurora-danger`, `--aurora-warning`, `--admin-accent{,-light,-dark}` (admin brand-red family), `--aurora-bg-rgb`, `--msg-scrim`. Exactly ONE arbitrary hex remains app-wide (`CateringPanel.tsx:205`). Dark mode via `.dark` on `<html>` (UserSettingsContext). **Messages header** purple gradient `#7e22ce → #7c3aed → #4f46e5` (mobile all modes + desktop dark) / light purple on desktop light. Message bubbles: own `--msg-own-bubble` `#E0E7FF`(light)/`#2D2F5E`(dark). Bubbles are uniform `rounded-2xl` (WhatsApp tail removed S63). **Incoming group-call ring screen** (S66): full-screen gradient `#0f0f1a→#1a1b2e→#2d2f5e`, pulsing purple avatar `#7e22ce→#4f46e5`, green Accept `#22C55E` / red Decline `#EF4444`.
- **Z-index scale (policy, S55):** 40 chrome / 55 click-away / 56 menus / 50 base modals / 70 stacked / 80 toasts / 200 messages-legacy-ceiling. New code uses the scale; off-scale migrates on touch.
- **Tone / working style:** concise and direct, minimal formatting, no over-bolding. Firestore hygiene: no `undefined` fields, explicit payload builders, null-guard `.data()`, error callbacks on every `onSnapshot`. The only remaining `cp` file-sync is `business.tsx ↔ main/business.tsx`. Tests gate deploy (270; `firebase.json` predeploy). Ship instructions must always say "web + Android" or "server-only".

*Updated July 6, 2026 (Session 68) — orientation + group-call ring/remote-video fix (device-confirmed) + companion 10b (messages I/O fully extracted, decomposition program CLOSED) + full security review (4C/6H/7M/3L) with rules/headers/log hardening SHIPPED to tree (not deployed) and the E2EE/KYC redesign STAGED. Nothing committed or deployed yet — see EXACT next steps above. Design system reaffirmed: "Modern Aurora" (Electric Indigo #6366F1 / Mint #10B981), NOT the stale Delta Navy/Red header claim.*

---

### Session 68 — CLOSE-OUT (Aug 12, 2026): committed + pushed, GitHub reconnected, standalone handoff prompt written

Wrap-up actions after the Session-68 body above:
- **Everything committed + pushed.** All Session-67/67b/68 work is on `origin/main` at commit **`53d6918`** ("session 68: 10b messages I/O extraction + security hardening + legal docs + handoff"). Working tree clean, local `main` == `origin/main`. **Still NOT deployed to Firebase** (rules + headers committed only; 10b not device-tested).
- **GitHub reconnected.** The prior PAT `sangam-push` had EXPIRED (that was the real cause of earlier push failures — reads worked because the repo is public, masking it). Created new fine-grained PAT **`sangam-push-2`** (repo `sangam-pwa-v2`, Contents: Read+write, **No expiration** per user choice) and stored it in the **macOS Keychain** (credential.helper osxkeychain) — the remote URL is now CLEAN (no embedded token). Deleted the expired `sangam-push` + `Ethnicity` tokens. Future pushes authenticate silently.
- **Legal deliverables** (four .docx: mutual+one-way × NDA+non-compete, eNoVo-US ↔ Company-B-India, with the India §27 caveat + neutral-seat arbitration) are in `../Legal_Agreements/` — a SIBLING of the app repo, NOT git-tracked. The security report `../eNoVo_Security_Review_07_05_2026.md` is likewise outside the repo.
- **NEW: standalone next-session handoff prompt** written to project root as **`HANDOFF08122026.md`** — a self-contained "paste as the first message of a fresh session" brief (project context, run/build/test commands, this session's state, prioritized next steps, decisions, gotchas, design system, and a read-first-then-confirm-before-editing instruction). It points back to THIS running note + the security report as source-of-truth. Keep both in sync going forward.

**Immediate next task for the next session (unchanged from §3 above):** deploy this session's committed work — `npm run ship` + Run ▶ + messages regression, then validate rules in the Console Playground and `firebase deploy --only firestore:rules`. Then the security backlog: H1 (KYC relocation) → C1+C2 (E2EE redesign). Do NOT advertise messaging as end-to-end encrypted until C1+C2 land.

*Updated Aug 12, 2026 (Session 68 close-out) — all work committed + pushed (`53d6918`); GitHub reconnected via new Keychain-stored PAT `sangam-push-2` (old one had expired); standalone `HANDOFF08122026.md` created for the next session. Nothing deployed to Firebase yet — deploying the committed rules/headers + shipping/device-testing 10b is the #1 next task.*


---

### Session 69 (Sep 2–3, 2026) — External-review remediation blitz: 9 of 10 findings CLOSED + exec Product Status Dashboard (built, hosted, standing-rule maintained) + NDA term refinements

One long continuous session (ran across a context compaction; treat as one). Written to be resumed cold.

**(1) What we were working on.** Five arcs, in order: (a) orientation from `HANDOFF08122026.md` (found it stale — the July rules had in fact been deployed Jul 5; verified against live console rather than trusting the note); (b) an **executive Product Status Dashboard** — read all 13 OneNote "eNoVo" pages + sub-pages via computer control, cross-checked every requirement against the codebase, and built a standalone single-file HTML dashboard (154 tracked features) — then restyled, production-hardened, and hosted it; (c) **NDA/non-compete refinements** (4 docx in `../Legal_Agreements/`); (d) the client shared an **external security review with 10 findings (C-01…L-10)** — we analyzed why our July review missed some, then **implemented and deployed nine of the ten** across four releases in ~24h: C-01 (private member data), C-03 (business-tier self-upgrade), H-05 (confidential reports + server-side 3-strike), H-06 (July's M2 headers deploy), H-04 (managed admin permissions), H-07 (complete account deletion), H-01 (tax-ID/KYC relocation), M-08 (password strength), L-10 (sign-in errors); (e) priorities set for the two remaining items (M-09 parked, C-02 blocked on a product decision).

**(2) Key decisions and WHY.**
- *Dashboard is deliberately OUTSIDE the app repo* (sibling folder `../eNoVo_Feature_Dashboard/` with its own `firebase.json` + `.firebaserc`), hosted as a **separate Firebase Hosting SITE `enovo-status` inside the same project `mithr-1e5f4`** — public link for execs with zero risk to the app's hosting config. Base code untouched, per explicit client requirement.
- *Dashboard design:* client pointed at a Gemini-made mock → adopted its palette: **teal `#0d4f47` primary + gold `#f2b52a` accent** on `#f2f4f7` app-canvas, Inter font, 3-tab app shell (Overview / Feature Tracker / Risks & Decisions), KPI tiles + per-module progress. (This palette is the DASHBOARD's own brand — the app itself stays "Modern Aurora" Indigo/Mint, see the unchanged-block below.) Production-hardening decision: **no Tailwind Play CDN** — compiled once via Tailwind CLI and inlined into the single file; **Lucide icons inlined as an SVG string map** (no CDN, works offline). Data model = one `FEATURES` array (fields `m`odule/`n`ame/`v`alue/`s`tatus/`c`riteria/`d`etail/`t`ag; statuses delivered|progress|pending|blocked); ALL KPIs/progress bars/module rollups compute at render — **edit entries only, never the numbers**.
- *Dashboard standing rule (client-mandated):* every feature/security change updates it in **three places**: the Mac file, the Claude artifact (republish same container path `/root/enovo-dashboard/artifact.html` + URL `https://claude.ai/code/artifact/fb1bc0f5-b4b5-49ae-9c71-8e59c1b9c77f`), and remind the client to run `cd ../eNoVo_Feature_Dashboard && firebase deploy --only hosting`. Mechanics + full rule live in desktop project memory `feature_dashboard.md`. NOTE: the artifact share-link pins a version — client must update the shared pin from the page's share menu after republish.
- *NDA terms (applied to all 4 docx):* 24-month initial term; early termination ⇒ non-compete runs 36 months; even after 36 months, competitor may launch a similar product but **not in countries we already launched**; mutual same-domain covenant; documentation/source-code covered on AND after execution; no retention of docs/source on any termination.
- *Why the July review missed things (analysis delivered):* it was a static rules/crypto-focused review with no dynamic pentest, and some planning-note claims (e.g. "strong passwords built") were never verified against code. Classified per finding: identified (C-02, H-06), partial (C-01, H-04, H-05, M-09), missed (C-03, H-07, M-08, L-10).
- *H-04 design — Firebase Auth custom claims, not email lists.* New CF `onAdminConfigWritten` (functions/src/index.ts) syncs `{admin:true}` claims from **`adminConfig/settings.adminEmails`** — a NEW collection because `appConfig`'s match is `read: if true` (that world-readability WAS the finding). Safety: **no-op on empty/missing list** so a bad write can never mass-revoke every admin. `isAdminUser()` in rules → `request.auth.token.admin == true` (no more per-request `get()` of a doc). Bootstrap order MATTERS (done, in order): deploy functions → create `adminConfig/settings` via console (bypasses rules; I drove Chrome for this) → CF stamps the claim (verified in Cloud Logging: `[adminClaims] granted admin to sarath.s1884@gmail.com`) → sign out/in (token refresh) → deploy rules → ship clients. Legacy `appConfig/settings.adminEmails` field left in place for OLD cached clients' UI gating — it grants nothing now; remove the field via console once confident all clients updated (STILL PENDING, see next steps).
- *H-07 design — delete vs anonymize.* Deletion sweep extended to every collection referencing the member; records other parties rely on are ANONYMIZED, not deleted (businessReviews, cateringOrders, cateringQuoteRequests/Responses → `"Deleted user"`, contact fields blanked) — and the delete-account modal now DISCLOSES that split. Two real bugs found and fixed en route: **marketplaceListings were never deleted** (sellerId), and the forum sweep queried subcollection `'likes'` which doesn't exist (it's `'forumLikes'`). Comments on others' content deleted via `collectionGroup('comments')` — needed a new collection-group READ rule (scoped `userId == request.auth.uid`) + a `comments.userId` COLLECTION_GROUP fieldOverride in `firestore.indexes.json`. The C-01 `users/{uid}/private/profile` subdoc is deleted BEFORE the profile doc so it can't orphan; ditto `businesses/{id}/private/kyc` (added when H-01 landed later the same session).
- *H-01 design — reuse the C-01 private-subdoc pattern.* Business KYC (tin/tinType/beneficialOwners/verificationDocs/photoIdUrl) → new owner/admin-only `businesses/{id}/private/kyc` (rules match added; admin read also covered by the existing `/{path=**}/private/` collection-group rule); user-doc KYC (tinNumber/tinValidationDetails/beneficialOwners/verificationDocUrls/photoIdUrl) → joins the existing `users/{uid}/private/profile`. **Lazy migration on sign-in** in AuthContext: user fields move with `deleteField()`, and a fire-and-forget pass migrates all businesses `ownerId == uid`. Admin panel + pending-registrations queue read private with public-doc fallback for unmigrated accounts. Public docs keep only non-sensitive flags (`tinValidationStatus`, `tinVerified`, `kycStatus`).
- *M-08 scope:* sign-up only — the app has NO in-app password change (resets go through Firebase's email flow, which we can't restyle), and sign-IN deliberately still accepts legacy 6+ passwords. Policy in new `src/utils/passwordStrength.ts`: 8+ chars, ≥3 of 4 character classes, ~60-entry common-password blocklist, no email-name inclusion; 0–4 score drives a live 4-segment meter on signup.
- *L-10:* full error-code map in login.tsx (user-disabled, network-request-failed, missing-password, invalid-login-credentials, internal-error, …) with actionable copy; unknown codes render friendly text + short code — **raw Firebase error strings never reach the member**.
- *M-09 parked deliberately:* App Check misconfiguration can lock out real clients; wants a staged rollout (register reCAPTCHA for web + Play Integrity for Android in console → monitor-only → verify traffic → enforce). Console work; next session.
- *Remaining order recommended & client-approved:* was H-01 → L-10+M-08 (one auth-screen pass) → M-09 staged → C-02 as its own project with the message-history migration decision made up front (migrate-and-re-encrypt vs archive-as-is). C-02 guardrail stands: **no "end-to-end encrypted" copy anywhere until it ships.**

**(3) Completed this session (ALL deployed + verified in production unless noted).**
- **Release 1 — C-01/C-03/H-05** (commits through `b940a2b`'s predecessors): private member-data subdoc + lazy migration; account-type immutability guard in rules; admin-only reports/moderationQueue reads + `onModerationQueueWritten` CF (server-side 3-strike auto-hide + author notification). Verified live: rules markers in console, queue doc reportCount:3 + escalatedAt, post isHidden, notification delivered ("Jaison" test).
- **H-06:** July's M2 security headers deployed with a ship; verified.
- **Release 2 — H-04** (`827719e`): CF + rules + AuthContext claims detection (hardcoded `ADMIN_EMAILS` deleted from `config.ts` incl. the catastrophic-catch fallback) + admin.tsx email mgmt → `adminConfig/settings`. Full bootstrap executed; claim grant verified in Cloud Logging; admin panel confirmed working post-rules.
- **Release 3 — H-07** (`b940a2b`): extended deletion sweep + anonymization + disclosure + rules deltas (recipient-scoped notification deletes — cateringNotifications was `delete: if false`) + comments fieldOverride index. Deployed (indexes + rules + ship).
- **Release 4 — H-01 + M-08 + L-10** (`372455b`, `ac39481`): as designed above. Deployed (rules + ship).
- **Android:** tablet (samsung SM-X230) reinstalled after every ship — I drove Android Studio Run ▶ via computer control each time (install #1+2 verified via toast/Logcat; #3 build verified fresh, toast unseen — see §4).
- **Dashboard:** built (154 features from OneNote×codebase), restyled teal/gold, Tailwind-CLI compiled + inlined, hosted on site `enovo-status`, published as Claude artifact, and **updated after every release** — now reads "Nine of the external review's ten findings closed", H-01/M-08/L-10/H-04/H-07 all Delivered, tax-ID item cleared from the red banner; only C-02 (blocked) + M-09 remain.
- **NDAs:** all six term changes applied + saved to the 4 docx.
- **Git:** remote confirmed at `b940a2b`; commits `372455b` + `ac39481` awaiting client push (see §5). All commits carry the session trailers.

**(4) In progress / half-done.**
- **`git push origin main`** pending for `372455b` (H-01) + `ac39481` (M-08+L-10) + this note's commit.
- **Dashboard hosting deploy** pending: `cd ../eNoVo_Feature_Dashboard && firebase deploy --only hosting` (Mac file + artifact already updated).
- **H-04 step 6 cleanup NOT done:** delete the legacy `adminEmails` field from `appConfig/settings` via console — safe now that the new client shipped, but wait a few days for cached clients if cautious.
- **Tablet install #3 (H-01/M-08/L-10 build)** visually unconfirmed: build fired and completed (fresh APK 19:04 local), but the client's Mac was in a full-screen Space so I couldn't read the install toast — client to glance at the tablet.
- **M-09** task created, not started. **C-02** blocked on the message-history migration decision. Messages full-regression on tablet (Session-68 10b checklist) still outstanding.
- **Artifact share-pin:** viewers of the shared dashboard link may see a pinned older version until the client updates the pin in the page's share menu.

**(5) EXACT next steps.**
1. Mac Terminal: `cd ~/ethniCity_03_19_2026/sangam-pwa-v2 && git push origin main` and `cd ~/ethniCity_03_19_2026/eNoVo_Feature_Dashboard && firebase deploy --only hosting`.
2. Glance at the tablet — confirm eNoVo relaunched after the last install.
3. In a few days: Firebase console → Firestore → `appConfig/settings` → delete the `adminEmails` field (grants nothing; UI-gating legacy only).
4. **M-09 (next session):** console → App Check → register web app (reCAPTCHA v3/Enterprise site key) + Android (Play Integrity; debug provider for dev builds) → wire `initializeAppCheck` in the client → run UNENFORCED, watch metrics → enforce on Firestore/Auth/Functions once traffic verifies. Then lockout messaging.
5. **C-02 (dedicated session):** first get the client's decision — migrate-and-re-encrypt old messages vs archive-as-is — then implement per the design in `../eNoVo_Security_Review_07_05_2026.md` §7. Until shipped: no E2EE marketing/in-app copy.
6. Backlog after that: H4-create notification callable, M4 transcribe guard, H6 ephemeral TURN, M-list ops items, Play Store launch, messages regression.

**(6) Important context / constraints / files.**
- *New/changed this session:* `functions/src/index.ts` (+`onAdminConfigWritten`, +`onModerationQueueWritten`), `firestore.rules` (many dated SECURITY comments — C-01/C-03/H-05/H-04/H-07/H-01), `firestore.indexes.json` (comments fieldOverride), `src/utils/passwordStrength.ts` (NEW), `src/services/dataPrivacy.ts` (sweep rewrite), `src/services/{auth,businessRegistration,moderation}.ts`, `src/contexts/AuthContext.tsx` (claims + two lazy migrations), `src/pages/{admin,profile,messages,feed,events,marketplace,housing}.tsx`, `src/pages/auth/{login,signup}.tsx`, `src/constants/config.ts` (ADMIN_EMAILS GONE), `android/app/build.gradle` (AGP 9.2.1 `proguard-android-optimize.txt`). Outside repo: `../eNoVo_Feature_Dashboard/` (index.html + firebase.json site `enovo-status` + README), `../Legal_Agreements/*.docx`. Firestore data: NEW `adminConfig/settings` doc = the admin email list (admin-only).
- *Workflow constraints (evolved since S68):* the device VM (`device_bash`) CAN commit (git works; stale `.git` locks need `device_request_delete_permission`, and that grant does NOT survive bridge reconnects — re-request each session); **push/pull only from the client's Mac Terminal** (PAT `sangam-push-2` in macOS Keychain); verify pushes from the cloud side via `git ls-remote` (repo public). **Never `npm install` and never `npm test` in the VM** (rollup darwin binary) — tests run on the Mac as the ship gate. **A classifier blocks me from deploying/editing production Firebase rules — the client runs all `firebase deploy` commands**; I CAN drive Chrome for console data-entry (created `adminConfig/settings` that way) and read Cloud Logging. Android Studio & Terminal are click-tier computer-use only (no typing; grants reset per session; the Gradle-sync banner can swallow the Run click — Sync Now first; verify installs via `find android/app/build -newermt "-1 minute"` + Logcat/toast). Deploys remain Mac-only and SEPARATE: `npm run ship` (hosting+Android assets) / `--only functions` / `--only firestore:rules` / `--only firestore:indexes`.
- *Desktop project memory files* (Claude app): `MEMORY.md`, `git_workflow.md`, `feature_dashboard.md` — keep in sync with this note.
- *Security posture now:* 9/10 external findings closed and verified; open = C-02 (E2EE rework, top blocked priority) + M-09 (App Check). July items still staged: H4-create callable, M4, H6.
