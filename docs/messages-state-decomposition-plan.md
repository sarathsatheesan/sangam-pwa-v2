...
Remaining after tranche 9: 16 ACTIVE useState declarations (metric corrected — prior 'N useState' counts included the import line + comments; use `grep -c 'const \[.*\] = useState'`). Domains 10 (core data + services/messages.ts) → 11 (E2EE/calls) remain.
